...
def test_api_doc_covers_top_level_public_exports_and_methods() -> None:
    ...
    for token in (
        ...
        "直接送信型の `snapshot()` は最後に正常送信した入力状態",
        "直接送信型でも、サブコマンド応答は自動送信",
        ...
        "await left.tap(Button.SR, Button.SL)",
        "SR+SL を送る必要があります",
        "「持ちかた/順番を変える」画面で単体 Joy-Con として順番登録",
        "OS、ドングル、ファームウェアをまたぐ互換性は未検証",
        "`None` を指定した場合は、既定周期",
        "各項目には独立した既定値があります",
...
def test_hardware_doc_separates_confirmed_unconfirmed_and_troubleshooting() -> None:
    ...
    for token in (
        ...
        "macOS 15.7.7 / CSR8510 A10 では Pro Controller の限定的な動作確認結果",
