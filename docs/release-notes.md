...
- 最低 Python バージョンを 3.12 から 3.13 へ引き上げました。Python 3.12 では `swbt-python==0.6.0` をインストールできません。Python 3.13 以上へ更新し、仮想環境を作り直してからインストールしてください。
- コントローラーの公開 API、HID レポート、周期送信、直接送信の実装は 0.5.4 から変更していません。`ReportSender`、`ReportLoop`、Bumble、`asyncio` の時計を差し替える補正も追加していません。

### 対応バージョンと検証

- CI のテスト対象を Python 3.13 / 3.14 に変更しました。Ubuntu と macOS で、静的解析、単体テスト、実機を使わない統合テスト、package build を実行します。
- Windows のローカル環境では Python 3.13.5 / 3.14.6 の両方で、451 件の単体テスト、165 件の実機を使わない統合テスト、package build を確認しました。Python 3.14 の Switch 実機接続は未確認です。

### 実機確認範囲

- [Issue #152](https://github.com/niart120/swbt-python/issues/152)の調査では、Windows 11 / CSR8510 A10 / WinUSB の同じ構成で、Python 3.12.10 では 8 ms ごとのジャイロ入力に周期的なカクつきが見られ、Python 3.13.5 では同じ入力が滑らかに見えました。Python 3.12.10 で Bumble 0.0.230 と 0.0.233 を比較しても症状は変わらず、Bumble 0.0.233 の HCI credit probe では queue 待ちが発生していません。
- この観測は対象 Windows 環境での結果です。Python 3.13 以上でも、OS 負荷を含む 8 ms 周期や 125 Hz の厳密な送信間隔を保証するものではありません。Python 3.14、別の OS、専用 USB Bluetooth ドングル、対象機器、ファームウェアでのジャイロ表示は未確認です。

## 0.5.4

### 修正と動作変更

- 周期送信型の `pair()`、`connect()`、`reconnect()`、`create_profile()` が、protocol ready 直後に残る自動入力抑止の終了を待ってから正常終了するようにしました。接続 API 完了直後の入力が初期化中の抑止に吸収される問題を修正します。
- 直接送信型は明示入力が自動入力抑止の対象外であるため、protocol ready で正常終了する従来の送信境界を維持します。接続確認用の入力レポートやレポートループは追加しません。
- 周期送信型は最新状態を周期送信する契約を維持します。すべての短時間入力や中間状態を必ず1回送信するキュー型の契約には変更していません。

### 実機確認範囲

- 通常入力準備境界の実装後、Windows 11、CSR8510 A10、WinUSB、Bumble 0.0.233 の構成で Pro Controller 相当の初回ペアリングを実行しました。周期送信型の接続 API 完了直後から 50 ms だけ Button A を入力し、A 付きの周期 `0x30` を 7 件と後続のニュートラル入力をトレースで確認しました。対象機器側の Button A 反映も利用者が目視確認しています。この実行では対象機器のモデルとファームウェアを再確認していません。
- active reconnect、Joy-Con、別の OS、専用 USB Bluetooth ドングル、対象機器、ファームウェアでの修正後の短時間入力は未確認です。確認済み構成と未確認範囲の全体は、[実機準備手順](hardware.md)を参照してください。
...
- 具象コントローラーの実装を `controllers.py` へ移し、共通の公開操作を `SwitchGamepad` へ集約しました。`ProController`、`JoyConL`、`JoyConR` と各直接送信型の import、コンストラクター引数、実行時の動作は v0.5.1 から変更していません。利用者コードの変更は不要です。
- HID リンク接続から初期化完了までのニュートラル入力送信を、非公開の `ProtocolHandshake` に分離しました。`ReportLoop` は初期化完了後の周期送信型だけが使います。直接送信型は初期化後に自動入力送信タスクを持たず、従来どおり `send()` と入力操作で送信します。
- テスト用の下位通信実装の差し替え経路と未使用のトレース履歴を整理しました。これらは公開 API ではありません。

### 互換性の検証

- 初期サブコマンドへの応答後、ライブラリが自動送信するニュートラル入力を 300 ms 抑制する既存の動作を維持します。この抑制は初期化中と周期送信型の自動入力に適用され、直接送信型の `send()` と入力操作は抑制しません。公開 API の変更はありません。

### 実機確認範囲

- 2026-07-25 に Windows 11、CSR8510 A10、WinUSB、Bumble 0.0.230、Python 3.13.5、専用 USB Bluetooth ドングルの組み合わせで、Pro Controller の初回ペアリングと再接続を比較しました。300 ms では周期送信型と直接送信型が接続、Button A の送信、ニュートラル入力による終了まで完了しました。100 ms では周期送信型の応答・自動送信が増加し、初回ペアリングでは Button A の反映を確認できませんでした。抑制なしでは両方式とも接続 API がタイムアウトし、初回ペアリングでは対象機器に認識されませんでした。
- この比較では、接続後に対象機器が新しいサブコマンドを送る場面を観測していません。300 ms の抑制を初期化中だけへ縮小できる根拠は得られていません。別の OS、専用 USB Bluetooth ドングル、対象機器、ファームウェアでの結果は未確認です。確認済み構成と未確認範囲の全体は、引き続き[実機準備手順](hardware.md)を参照してください。
...
- 2026-07-24 に Windows 11、CSR8510 A10、WinUSB の組み合わせで、Pro Controller、Joy-Con L、Joy-Con R の初回ペアリングと再接続が、初期サブコマンドへの応答とプレイヤー割り当ての完了後に正常終了することを確認しました。この実行では Switch 2 のファームウェアを再確認していません。
- 同じ構成で、Joy-Con L/R が登録用の SR+SL 入力を追加送信せずに初期化を完了することを確認しました。
- 同じ構成の Pro Controller で、周期送信型で作成したプロファイルを直接送信型で再利用できることと、直接送信型が初期化完了後に内部の周期送信を停止することを確認しました。Joy-Con L/R の方式間再利用と、別の OS、専用 USB Bluetooth ドングル、対象機器のファームウェアでは未確認です。
- 直接送信型 Pro Controller では、入力操作後の観測中に、`close()` の開始前から対象機器へ入力が反映されることを確認しました。
