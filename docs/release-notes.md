...
- 今回の通常入力準備境界を実装した後、Windows 11、CSR8510 A10、WinUSB、Bumble 0.0.233 の構成でPro Controller相当の初回ペアリングを実行しました。周期送信型の接続 API 完了直後から50 msだけButton Aを入力し、A付きの周期`0x30`を7件と後続のニュートラル入力をトレースで確認しました。対象機器側のButton A反映も利用者が目視確認しています。この実行では対象機器のmodelとfirmwareを再確認していません。
