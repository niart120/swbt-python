...
Joy-Con L/R は Windows 11 / CSR8510 A10 / WinUSB / Switch 2 firmware 22.1.0 で限定的な観測があります。Joy-Con L は SR+SL 登録、利用者指定色、active reconnect 後の D-pad 入力を確認済みです。Joy-Con R は SR+SL 登録、`0x22` ACK 互換処理、active reconnect 後の ABXY 入力、利用者指定色を確認済みです。左右のスティックは hold / circle の送信まで確認し、横持ち Joy-Con の UI 制約により補正画面の完了は確認済み範囲に含めません。確認済み範囲を越える Joy-Con profile の実機互換性は保証せず、検証状態は `docs/hardware.md` と `spec/hardware-test-log.md` を正本にします。
