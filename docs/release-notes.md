...
- `pair()`、`connect()`、`reconnect()`、`create_profile()` は、HID リンク接続だけで完了せず、初期サブコマンドへの応答とプレイヤー割り当てが完了してからコントローラーオブジェクトを返すようになりました。Joy-Con L/R は登録用の SR+SL 入力を利用者が追加送信する必要はありません。
