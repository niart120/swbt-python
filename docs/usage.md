...
`pair()`、`connect()`、`reconnect()` が正常終了した時点で通常入力を開始できます。周期送信型は、接続初期化中の自動入力抑止が終わってから戻ります。固定時間の待機や `status().report_counters` の増加待ちは不要です。周期より短い複数の状態更新は最新状態へまとめられるため、すべての中間状態を1回ずつ送る API ではありません。
