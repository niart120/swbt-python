...
Joy-Con profile の実機互換、SDP 完全一致、OS / dongle / firmware をまたぐ互換性は未検証です。2026-07-06 の Joy-Con L 実機観測では、HID 通信上の device name と device-info reply は Joy-Con L になり、pairing 自体も完了しましたが、Switch の登録 toast は Pro Controller のままで、コントローラーの順番画面は Joy-Con L として表示されました。
...
custom transport は `HidDeviceTransport` を満たす object として `SwitchGamepad(transport=...)` に渡します。`open()`、`start_advertising()`、`close()`、interrupt/control send、callback registration、bonded peer listing、active reconnect を transport 側で実装します。Bluetooth HID Device としての local controller address を持つ transport は、`local_bluetooth_address()` で 6 bytes を返してください。address を提供できない場合は `None` を返します。
