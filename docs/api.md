...
custom transport は `HidDeviceTransport` を満たす object として concrete controller の `transport=...` に渡します。`open()`、`start_advertising()`、`close()`、interrupt/control send、callback registration、bonded peer listing、active reconnect を transport 側で実装します。Bluetooth HID Device としての local controller address を持つ transport は、`local_bluetooth_address()` で 6 bytes を返してください。address を提供できない場合は `None` を返します。

concrete controller は injected transport に `adapter` や `key_store_path` を後から設定しません。key store が必要な custom transport は、その constructor で設定を受けてください。
