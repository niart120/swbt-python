...
- Linux / macOS は experimental として、手順を文書化する
...
Linux / macOS は experimental として手順を出す。ただし、動作検証済みとは書かない。macOS CI の実ハードなし gate は、macOS runner 上で依存関係のインストール、単体テスト、fake transport を使った結合テスト、package build が壊れていないことだけを確認する。USB Bluetooth dongle、OS Bluetooth stack 設定、HID advertising、pairing、input reflection は検証しない。
