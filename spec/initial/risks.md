...
- Linux / macOS は experimental として準備候補を文書化する
- Linux / macOS の adapter open、pairing、input reflection は確認済み範囲へ含めない
- macOS CI は実ハードなしの dependency sync、unit tests、fake transport integration tests、package build に限定する
...
初期 release gate では、確認済み構成と未確認構成を分けて扱う。確認済み範囲は `docs/hardware-test-log.md` の実機観測に限り、未確認構成へ一般化しない。
...
この構成では、pairing、L2CAP、full observed subcommand handshake、Button A、neutral、D-pad、left / right stick、active bond reuse reconnect を観測済みとする。L+R 同時押し表示は、Switch button check UI の表示制約として扱う。

### 11.3 未確認範囲

- Linux / macOS の adapter listing、adapter open、OS 設定変更。
- Linux / libusb permission、udev rule、kernel / BlueZ 競合解消。
- macOS の外付け HCI 設定と libusb 準備。
...
Linux / macOS は experimental として準備候補を出すが、supported claim にはしない。macOS CI の no-hardware gate は、macOS runner 上で依存解決、unit tests、fake transport integration tests、package build が壊れていないことだけを確認する。USB Bluetooth dongle、OS Bluetooth stack 設定、HID advertising、pairing、input reflection は検証しない。
