...
`SubcommandResponder` は configured `ControllerProfile` を参照して fixed identity / SPI seed / Device Info を生成する。report mode、IMU enable、vibration enable など Switch host の要求で変わる値は mutable `SubcommandSessionState` に分け、profile や `InputState` には置かない。
