...
- Periodic は `ReportLoop` が一定周期で `InputStateStore` の snapshot から `0x30` を生成する
- Direct は `send()` または意味的入力操作ごとに候補 state の `0x30` を1件送る
- Direct は transport 送信が完了した場合だけ候補 state を commit する
...
Direct の `neutral()` は接続済みを要求し、ニュートラル入力レポートを1件送ってから current state を確定する。送信失敗では最後に正常送信した state を維持する。
...
- Periodic / Direct の全 concrete controller は `profile_path` 内の namespace map を exp local identity の reconnect storage として使う。Joy-Con の native-address 経路だけは `key_store_path` を使う
...
- Direct の `send()`、意味的入力操作、`tap()` は input operation lock で直列化し、送信成功後の commit まで同じ operation とする
