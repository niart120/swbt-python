...
|---|---|---|
| `pair(timeout=..., key_store_path=...)` | 初回 pairing | HID advertising / connectable を開始し、incoming 接続を待つ |
| `reconnect(timeout=..., key_store_path=...)` | 保存済み bond だけを使う再接続 | 保存済み peer address / link key を使って active bond reuse reconnect を試行する |
| `try_reconnect(timeout=..., key_store_path=...)` | 再接続診断 | `ConnectionResult` で `no_bond`、`ambiguous_bond`、`timeout`、`failed` を返す |
| `connect(timeout=..., allow_pairing=False, key_store_path=...)` | 通常利用 | bond があれば `reconnect()` を優先する。bond がなく、`allow_pairing=True` の場合だけ `pair()` へ進む |
| `try_connect(timeout=..., allow_pairing=False, key_store_path=...)` | 接続診断 | `ConnectionResult` で reconnect / pairing fallback の結果を返す |
...
- `key_store_path` は `pair()` / `connect()` / `reconnect()` / `try_connect()` / `try_reconnect()` の接続時引数として設定できる
