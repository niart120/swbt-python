...
`list_bonded_peers()` と `connect_bonded_peer()` は、active reconnect のための最小境界である。`configure_key_store_path()` は、接続試行ごとに選んだ JSON key store を transport へ渡す。constructor 固定の key store にはしない。
