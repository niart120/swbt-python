...
`list_bonded_peers()` は current namespace だけを見る。current entries が 0 件なら空 tuple、1 件ならその peer、2 件以上なら unsupported shape として `InvalidKeyStoreError` を投げる。JSON entry の順序や最終更新推定で current peer を選ばない。
