...
| `0x04` | trigger buttons elapsed time | Pro は L/R、Joy-Con L/R は SL/SR に 300 tick を返す |
...
`0x03`、`0x30`、`0x40`、`0x48` の要求状態は `ControllerProfile` ではなく、接続単位の`SwitchHidSessionState`に保持する。`ControllerProfile`はfixed identity / protocol profileのみを表し、Switch hostからの要求で変わるreport mode、player lights、IMU mode、vibration enableを持たない。SPI `0x10` readはsession stateを変更しない。
...
接続 session は、supported report mode `0x30` と 0 以外の player lights が揃った場合だけ
protocol ready になる。到着順と他の subcommand の受信集合は固定しない。
`0x30 SET_PLAYER_LIGHTS` は simple ACK `0x80` を返す。`0xb0` は別 subcommand の
`0x31 GET_PLAYER_LIGHTS` が player lights 1 byte を返す場合の ACK であり、
`0x30` の実装差ではない。ready 判定は ACK byte ではなく、同一 session で受理して
reply を送信した report mode と player lights の状態を使う。
