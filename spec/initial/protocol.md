...
protocol ready になる。到着順と他の subcommand の受信集合は固定しない。`0x30` ACK は
現行 swbt-python / joycontrol と同じ `0x80` を維持する。swbt-daemon の `0xb0` との
実装差は監査記録に残し、ready 判定に ACK byte の一致を使わない。
