...
- IMU payloadは36 bytesとし、hostのIMU mode `0x01`ではint16 little-endianの6軸値を3 frame、`0x02-0x05`ではquaternion packing mode 2を持つ
...
| `0x40` | sensor sleep / IMU mode | modeをsession stateへ記録してackを返し、`0x01`は標準形式、`0x02-0x05`はquaternion形式を選ぶ |
...
`0x03`、`0x40`、`0x48` の要求状態は `ControllerProfile` ではなく `SubcommandSessionState` に保持する。`ControllerProfile` は fixed identity / protocol profile のみを表し、Switch host からの要求で変わる report mode、IMU enable、vibration enable を持たない。
