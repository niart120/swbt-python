...
`controller_colors=None` は既定の `ControllerColors(body=0x0D0D0D, buttons=0xFFFFFF)` を使う。`left_grip` と `right_grip` を省略した場合は `body` と同じ色として扱う。色は作成時に固定し、接続後の `set_color()`、`controller_colors=` setter、profile mutation API は提供しない。
...
`body`、`buttons`、`left_grip`、`right_grip` は 24-bit RGB integer として扱う。`left_grip` と `right_grip` が `None` の場合は `body` と同じ値に正規化する。`ControllerColors(body=0x112233, buttons=0x445566, left_grip=0x778899, right_grip=0xAABBCC)` は SPI `0x6050` から `11 22 33 44 55 66 77 88 99 aa bb cc` として返る。範囲外値、文字列、bytes、tuple は `InvalidInputError` とする。
