...
Linux / macOS の USB Bluetooth adapter 対応を、確認済み対応ではなく experimental として扱う。USB Bluetooth dongle、OS の Bluetooth stack 設定、Nintendo Switch 実機での検証はこの unit の完了条件に含めない。repo 側で行うのは、Bumble の USB transport 前提に沿った事前確認の文書化、no-open diagnostics の整備、実ハードなしの macOS CI、未確認範囲の明示までとする。
...
| macOS user | hardware guide を読む | 手順と未確認リスクが分かる | NVRAM など OS 設定を保証済み手順として扱わない |
...
| maintainer | release docs を確認する | package metadata は Windows / Linux / macOS の配布対象を示し、public docs は Windows 確認済み構成と Linux/macOS experimental を分ける | classifier を実機確認済みの根拠として扱わない |
...
- Linux / macOS を Windows と同じ実機確認済み構成として扱うこと。
...
| OS / driver / adapter | required | not-run-by-policy | Linux/macOS 上の adapter listing、adapter open、pairing、input reflection はこの unit では実行しない。macOS CI は実ハードなしの範囲に限定する |

### 未解決事項

- Linux の具体的 udev rule と BlueZ 解放手順は、現時点では文書化された手順であり、repo の確認済み手順ではない。
- macOS の外付け HCI を OS Bluetooth stack に掴ませない設定は、現時点では文書化された手順であり、repo の確認済み手順ではない。
...
| preparation guidance | Linux / macOS setup text | Bumble docs で確認できること、事前確認、未検証事項を分ける | 保証文にしない |
...
| CI scope control | workflow matrix | macOS job は実ハードなしで実行し、adapter open を含む command を入れない | 実OSでの adapter 動作保証ではない |
| package metadata | `pyproject.toml` classifiers | Windows / Linux / macOS の OS classifier を含む | classifier は配布対象 OS の表示に留め、実機確認済み範囲は README と hardware guide で分ける |
...
|---|---|---|---|---|---|
| green | `docs/hardware.md` が Linux / macOS を `experimental` として扱い、supported と書かない | docs regression | unit | no | `test_public_docs.py` で Linux/macOS の status と対応確認済み表現の不在を確認 |
| green | `docs/hardware.md` が Linux / macOS の手順と未確認事項を分ける | docs regression | unit | no | NVRAM や BlueZ 解放は確認済み手順として書かない |
...
| green | CI が `macos-latest` で実ハードなしの gate を実行する | regression | unit / integration | no | `matrix.os` に `ubuntu-latest` と `macos-latest` を追加。unit、fake integration、build を実行 |
...
| green | `pyproject.toml` が Windows / Linux / macOS の OS classifier を含む | regression | unit | no | `tests/unit/test_package_metadata.py` で classifier を確認。Linux/macOS の実機検証済み表現は docs 側で避ける |
...
| deferred | Linux/macOS で pairing / reconnect / input reflection を確認する | characterization | hardware | yes | この unit では実行しない。対応確認済みと書くための前提になる |
...
- Linux/macOS の OS 設定は、repo が実行して確認した事実ではない。文書では「Bumble の USB transport を使う前に確認すること」として扱う。
...
- package metadata の OS classifier は、install と CI の対象 OS を利用者に示す。Linux/macOS の実機確認済み範囲は classifier では表現せず、README と Hardware Guide で experimental と明記する。
...
| `tests/unit/test_package_metadata.py` | modify | Windows / Linux / macOS の OS classifier contract を固定 |
| `.github/workflows/ci.yml` | modify | macOS runner を実ハードなしの CI gate に追加 |
...
| `pyproject.toml` | modify | Linux / macOS OS classifier を追加 |

## 10. 検証

| command | result | notes |
|---|---|---|
| `uv run pytest tests/unit/test_public_docs.py::test_hardware_doc_separates_confirmed_unconfirmed_and_troubleshooting tests/unit/test_readme_docs.py::test_readme_documents_confirmed_and_unconfirmed_hardware tests/unit/test_readme_docs.py::test_readme_keeps_detailed_hardware_and_key_store_guidance_in_docs tests/unit/test_package_metadata.py::test_package_metadata_includes_linux_and_macos_os_classifiers tests/unit/test_ci_workflow.py -q` | pass | docs / README / CI / package metadata の experimental 境界を確認 |
| `uv run pytest tests/unit/test_package_metadata.py -q` | pass | Windows / Linux / macOS の OS classifier を確認 |
...
| `uv run pytest tests/unit/test_public_docs.py tests/unit/test_readme_docs.py tests/unit/test_release_gate_docs.py tests/unit/test_hardware_test_log_docs.py -q` | pass | 17 passed。公開 docs から内部向けの硬い表現を外した後に再確認 |
| public docs wording search | pass | README、hardware docs、hardware matrix、risks から利用者向けではない旧表現を削除。根拠監査の formal な分類名だけは spec に残す |
...
| macOS adapter command | not run | macOS CI は実ハードなしの範囲に限定し、adapter listing / open / OS Bluetooth stack 設定変更は行わない |
...
- Linux/macOS を実機確認済み構成として扱う package metadata や docs 表現。
- OS 別の udev rule、BlueZ 操作、macOS NVRAM 設定の確定手順化。
- macOS CI で実ハードなしの gate を越えて adapter を開く検証。
...
- [x] macOS CI で保証する範囲を実ハードなしの CI gate として記録した
- [x] public docs と README の experimental 表現を更新した
- [x] GitHub Actions に実ハードなしの macOS CI gate を追加した
