...
| Periodic controller | send が1周期を超えて遅延する | 過去 deadline を飛ばし、将来の最初の deadline まで待つ | 遅延 tick を burst 追送しない |
...
- USB Bluetooth dongle、pairing、advertising、Switch-facing report loop を使う実機実行。
...
| OS / driver / adapter | required | done | unit_063 の Windows / CSR8510 A10 観測は問題の起点にだけ使い、性能値を他環境へ一般化しない。新契約は fake clock 上の deadline 計算に限定する。 |
...
| 同 2 tests の GREEN 再実行 | pass | 2 passed。overrun 後は過去 deadline を飛ばし、将来の最初の deadline まで待機 |
...
| 項目 | 内容 |
|---|---|
| 実機要否 | not required。Issue 完了条件の「制御可能な計測」を fake clock unit test で満たす |
| 承認範囲 | none。dongle、pairing、advertising、Switch-facing report loop は実行しない |
| adapter | not applicable |
| 実行遮断 | 環境変数による遮断は採用しない。実機検証へ拡張する場合は、明示承認、対象 adapter、command、cleanup plan を別途確認する |
| log / artifact | unit test result と本仕様の検証欄 |
| cleanup | not applicable |

## 13. 先送り事項

- 実機 8 ms 設定での completion interval、長時間安定性、Switch 側反映時刻は未検証。必要なら `hardware-harness` の承認境界を通す別作業として扱う。
