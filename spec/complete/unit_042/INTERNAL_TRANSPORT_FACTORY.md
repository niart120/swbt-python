...
| Bumble / transport | required | done | Bumble behavior、SDP、L2CAP、adapter behavior は変更しない。公開境界 audit として root export、public docs、public docstring contract から transport extension 型を削除した |
...
| green | docs が custom transport を public extension point と説明しない | regression | docs | no | migration section は例外 |
...
| `spec/complete/unit_042/INTERNAL_TRANSPORT_FACTORY.md` | move | 完了した作業仕様 |
...
| `uv run pytest tests/unit -q` | pass | `350 passed`。root `HidDeviceTransport` xfail を解消 |
...
| `uv run pytest tests\unit\test_public_docs.py::test_api_doc_covers_top_level_public_exports_and_methods tests\unit\test_public_docs.py::test_rearchitecture_records_transport_removal_as_breaking_change -q` | red | API docs が custom transport extension point をまだ説明していた |
| `uv run pytest tests\unit\test_public_docs.py::test_api_doc_covers_top_level_public_exports_and_methods tests\unit\test_public_docs.py::test_public_docs_do_not_carry_stale_or_placeholder_wording tests\unit\test_public_docs.py::test_rearchitecture_records_transport_removal_as_breaking_change tests\unit\test_public_docs.py::test_agent_brief_keeps_generation_on_implemented_public_api -q` | pass | `4 passed`。user-facing docs から custom transport extension point を削除し、transport は内部境界として説明 |
| `uv run ruff check docs\api.md docs\agent-brief.md tests\unit\test_public_docs.py` | pass | `All checks passed!` |
...
- fake transport injection helper は `src/swbt/_testing/gamepad.py` に置いた。root export はしない。
