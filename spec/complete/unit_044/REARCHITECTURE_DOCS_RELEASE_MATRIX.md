...
| current docs | 現行 docs は `ProController`, `JoyConL`, `JoyConR` を案内し、old API は通常説明から除外している | `README.md`, `docs/api.md`, `docs/usage.md`, `docs/agent-brief.md` |
| release note | rearchitecture breaking change と migration table、version target を記録する | `docs/release-notes.md`, `pyproject.toml`, `uv.lock` |
...
| OS / driver / adapter | required | done | `docs/hardware.md` と `spec/hardware-test-log.md` の既存観測を基に、controller profile 別 matrix と release note の hardware scope を記録した。新規実機検証は行っていない |
...
|---|---|---|---|---|---|
| done | README の通常説明が new controller API を使う | regression | docs | no | `tests/unit/test_readme_docs.py` |
| done | API docs が `SwitchGamepad` を型として説明する | new | docs | no | `tests/unit/test_public_docs.py` |
| done | usage / examples が `ProController`, `JoyConL`, `JoyConR` を使う | regression | docs / integration | no | `tests/unit/test_public_docs.py`, `tests/integration/test_examples.py` |
| done | hardware matrix が Pro / Joy-Con L / Joy-Con R を分ける | new | docs | no | `tests/unit/test_public_docs.py` |
| done | release note に breaking change と migration table がある | new | docs | no | `tests/unit/test_release_gate_docs.py` |
...
| `docs/hardware.md` | modify | controller profile verification matrix |
| `examples/` | existing | new API examples |
| `spec/hardware-test-log.md` | existing | verified / unverified state |
| `docs/release-notes.md` | add | breaking change note |
| `pyproject.toml` | modify | version bump to `0.2.0` |
| `uv.lock` | modify | package metadata version bump |
| `mkdocs.yml` | modify | release notes を public docs nav に追加 |
| `spec/complete/unit_044/REARCHITECTURE_DOCS_RELEASE_MATRIX.md` | move | 完了した作業仕様 |

## 10. 検証

| command | result | notes |
|---|---|---|
| `uv sync --dev` | pass | standard dev gate |
| `uv run ruff format --check .` | pass | 90 files already formatted |
| `uv run ruff check .` | pass | All checks passed |
| `uv run ty check --no-progress` | pass | All checks passed |
| `uv run pytest tests/unit -q` | pass | 356 passed |
| `uv run pytest tests/integration -q` | pass | 93 passed |
| `uv sync --dev --group docs` | pass | docs dependency group installed |
| `uv run mkdocs build --strict` | pass | Documentation built successfully |
