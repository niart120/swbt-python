...
| `tests/unit/test_public_docs.py` | modify | breaking change 方針の characterization test |
| `spec/rearchitecture/05-milestones-implementation.md` | modify | unit mapping の参照更新 |
| `spec/complete/unit_038/REARCHITECTURE_DECISION_BOUNDARY_TESTS.md` | add / move | 作業仕様 |
...
| `uv run pytest tests/unit/test_package_import.py tests/unit/test_public_api_boundary.py tests/unit/test_public_docs.py -q` | pass | `32 passed, 4 xfailed`。xfail は unit_040 で green にする target boundary tests |
| `uv sync --dev` | pass | Resolved 53 packages / checked 41 packages |
| `uv run ruff format --check .` | red | `tests/unit/test_public_docs.py` が reformat 対象 |
| `uv run ruff check .` | red | `tests/unit/test_package_import.py` の import block formatting |
| `uv run ruff format tests\unit\test_public_docs.py tests\unit\test_package_import.py tests\unit\test_public_api_boundary.py` | pass | 1 file reformatted |
| `uv run ruff check --fix tests\unit\test_package_import.py` | pass | 1 fix applied |
| `uv run ruff format --check .` | pass | 78 files already formatted |
| `uv run ruff check .` | pass | All checks passed |
| `uv run ty check --no-progress` | pass | All checks passed |
| `uv run pytest tests/unit -q` | pass | `334 passed, 4 xfailed`。xfail は unit_040 で green にする target boundary tests |
...
- `xfail(strict=True)` の target boundary tests は unit_040 で green にし、xfail marker を外す。
