...
|---|---|---|---|---|---|
| green | 初回 `update(peer, keys)` は current だけを保存し previous を作らない | new | unit | no | `test_swbt_json_key_store_initial_update_writes_current_without_previous` で JSON namespace と peer entry を固定した |
| green | 同じ peer の 2 回目 `update()` は旧 current を previous に退避して新 current を保存する | new | unit | no | `test_swbt_json_key_store_overwrite_moves_old_current_to_previous` で old/new key marker を固定した |
| green | 3 回目以降の `update()` は previous を直前 current に置き換え、履歴を増やさない | edge | unit | no | `test_swbt_json_key_store_keeps_only_one_previous_generation` で previous namespace が 1 世代だけであることを固定した |
| green | `get(peer)` と `get_all()` は current のみを返し previous を通常 reconnect 候補に混ぜない | regression | unit | no | `test_swbt_json_key_store_reads_legacy_bumble_json_as_current` と `test_bumble_list_bonded_peers_ignores_previous_key_store_generation` で固定した |
| green | previous 退避または current 保存の失敗は key material を出さず diagnostics に記録される | edge | unit | no | `test_swbt_json_key_store_diagnostics_records_previous_generation_without_key_material` と既存 failure test で key material 不在を確認した |
| green | `BumbleHidTransport` は `key_store_path` 指定時に current / previous 対応 key store を使う | new | unit | no | `test_bumble_initialize_device_configures_json_key_store` を更新し、Bumble config ではなく `device.keystore` 差し替えを固定した |
| green | `SwitchGamepad(key_store_path=...)` の run metadata は current path と previous availability を secret なしで記録する | new | integration | no | `test_key_store_path_is_recorded_in_run_metadata` と `test_key_store_previous_generation_is_recorded_in_run_metadata` で固定した |
| green | active reconnect は previous が存在しても current の 1 peer だけを選択する | regression | unit | no | `test_bumble_list_bonded_peers_ignores_previous_key_store_generation` で transport の bonded peer listing が current のみ返すことを確認した |
| green | legacy Bumble JSON file は current として読める | regression | unit | no | `test_swbt_json_key_store_reads_legacy_bumble_json_as_current` で既存 namespace を current として読めることを確認した |
| deferred | 実機 incoming 再登録で current / previous 退避 event が trace に残るか確認する | characterization | hardware | yes | 実機承認が必要なためこの unit では未実行。controller search / change grip order screen 条件は pairing-free bond reuse と扱わない |
...
| `src/swbt/transport/` | not changed | Bumble-compatible key store helper は `swbt.transport.bumble` 内に閉じ込めた |
...
| `tests/hardware/test_reconnect_keystore.py` | not changed | 実機 characterization は未実行 |
| `docs/hardware-test-log.md` | not changed | 実機観測を追加していない |
| `spec/complete/unit_016/JSON_KEY_STORE_CURRENT_PREVIOUS.md` | modify | TDD 状態、検証、実機条件を更新 |

## 10. 検証

| command | result | notes |
|---|---|---|
| `uv run pytest tests\unit\test_bumble_transport.py::test_swbt_json_key_store_initial_update_writes_current_without_previous -q` | red then pass | red は `_CurrentPreviousJsonKeyStore` 未実装。実装後は 1 passed |
| `uv run pytest tests\unit\test_bumble_transport.py::test_swbt_json_key_store_overwrite_moves_old_current_to_previous -q` | pass | 1 passed。2 回目 update の previous 退避を確認した |
| `uv run pytest tests\unit\test_bumble_transport.py::test_swbt_json_key_store_keeps_only_one_previous_generation -q` | pass | 1 passed。previous が 1 世代だけであることを確認した |
| `uv run pytest tests\unit\test_bumble_transport.py::test_bumble_initialize_device_configures_json_key_store -q` | red then pass | red は古い `DeviceConfiguration.keystore == JsonKeyStore:<path>` 前提。更新後は 1 passed |
| `uv run pytest tests\unit\test_bumble_transport.py::test_swbt_json_key_store_diagnostics_records_previous_generation_without_key_material -q` | red then pass | red は `generation` / `previous_saved` 不足。実装後は 1 passed |
| `uv run pytest tests\unit\test_bumble_transport.py::test_bumble_key_store_update_failure_is_recorded_without_key_material -q` | pass | 1 passed。既存 failure diagnostics に余計な secret field が出ないことを確認した |
| `uv run pytest tests\integration\test_switch_gamepad_fake_transport.py::test_key_store_path_is_recorded_in_run_metadata tests\integration\test_switch_gamepad_fake_transport.py::test_key_store_previous_generation_is_recorded_in_run_metadata -q` | red then pass | red は `key_store_previous_exists` 不足。実装後は 2 passed |
| `uv run pytest tests\unit\test_bumble_transport.py::test_swbt_json_key_store_reads_legacy_bumble_json_as_current -q` | pass | 1 passed。既存 Bumble JSON file を current として読めることを確認した |
| `uv run pytest tests\unit\test_bumble_transport.py::test_bumble_list_bonded_peers_ignores_previous_key_store_generation -q` | pass | 1 passed。previous namespace を bonded peer 候補に混ぜないことを確認した |
| `uv run pytest tests\unit\test_bumble_transport.py -q` | pass | 34 passed。Bumble transport unit regression を確認した |
| `uv run pytest tests\integration\test_switch_gamepad_fake_transport.py::test_key_store_path_is_recorded_in_run_metadata tests\integration\test_switch_gamepad_fake_transport.py::test_key_store_previous_generation_is_recorded_in_run_metadata tests\integration\test_switch_gamepad_fake_transport.py::test_reconnect_records_bonded_peer_selection_without_advertising -q` | pass | 5 passed。run metadata と reconnect peer selection の局所 regression を確認した |
...
| `uv run pytest tests\unit -q` | pass | 128 passed |
| `uv run pytest tests\integration -q` | pass | 48 passed |
