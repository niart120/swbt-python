...
| 項目 | 要否 | 状態 | 根拠 / 理由 |
|---|---|---|---|
| Switch HID / report bytes | required | done | M1 は M0 / unit_009 で監査済みの `0x30` / `0x21` bytes を送信順序で使う。新しい byte layout は追加していない |
...
| green | `set_input()` が snapshot と次の periodic report に反映される | new | integration | no | `test_set_input_updates_snapshot_and_next_periodic_report` で固定 |
| green | `neutral()` が snapshot と次の periodic report を neutral に戻す | new | integration | no | `test_neutral_updates_snapshot_and_clears_next_periodic_report` で固定 |
...
| `uv run pytest tests\integration\test_switch_gamepad_fake_transport.py::test_set_input_updates_snapshot_and_next_periodic_report -q` | pass | 1 passed。`set_input()` が snapshot と periodic report に反映されることを確認した |
| `uv run pytest tests\integration\test_switch_gamepad_fake_transport.py::test_neutral_updates_snapshot_and_clears_next_periodic_report -q` | pass | 1 passed。`neutral()` が snapshot と periodic report を neutral に戻すことを確認した |
...
| `uv run pytest tests\unit tests\integration -q` | pass | 81 passed。unit と fake transport integration を確認した |
...
| `uv run pytest tests\unit -q` | pass | 68 passed |
| `uv run pytest tests\integration -q` | pass | 13 passed |
...
- [x] fake transport と report loop の実装を完了した
- [x] M1 の対象 test と型 gate を実行し、検証欄を結果で更新した
