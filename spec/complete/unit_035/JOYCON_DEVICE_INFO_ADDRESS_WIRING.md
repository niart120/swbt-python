...
| in-place address update | transport address が後から判明する | 既存 `SubcommandResponder` の session state と wrapper を保持したまま Device Info address だけ更新する | hardware probe wrapper を外さない |
...
| green | Device Info address 更新で responder を作り直さず session state を保持する | new | unit | no | `tests/unit/test_subcommand_responder.py` |
| hardware-pass | hardware test が configured local address と Device Info address の一致を要求する | new | hardware | yes | 2026-07-06 SDP policy retest で `device_info_data=04000102001bdcf99f7d0101` と configured local address `001bdcf99f7d` を確認 |
...
address 更新時に `SubcommandResponder` を作り直すと、session state は引き継げても hardware test の `RecordingDeviceInfoResponder` のような wrapper を外してしまう。実装は既存 responder に対して address だけを更新する。これは互換性維持のための特殊分岐ではなく、subcommand session owner を安定させるための境界整理である。

Device Info address だけでは Pro Controller toast / Joy-Con 登録未完了を解消できなかった。後続 unit_036 で Joy-Con SDP policy を変更した retest では、同じ Device Info address を返したうえでユーザ目視の Joy-Con 登録が成功した。この unit の完了判定は、Device Info address wiring が transport local address と一致し、responder wrapper を外さず hardware test が観測できる状態になったことに置く。
...
| `src/swbt/protocol/subcommand.py` | modify | responder の Device Info address in-place 更新 |
...
| `tests/unit/test_subcommand_responder.py` | modify | address 更新時の session state 保持 |
...
| `uv run pytest tests/unit -q` | pass | 327 passed |
...
| `uv run pytest tests/unit/test_subcommand_responder.py::test_device_info_subcommand_updates_bluetooth_address_without_new_responder tests/unit/test_bumble_transport.py::test_bumble_start_advertising_refreshes_local_bluetooth_address_after_power_on tests/integration/test_switch_gamepad_fake_transport.py::test_pair_refreshes_transport_bluetooth_address_after_advertising_for_device_info -q` | pass | 3 passed |
...
| hardware Joy-Con L retest, `build\hardware\joycon-left-device-info-address-20260706-034000` | fail | Device Info reply payload was `04 00 01 02 00 1b dc f9 9f 7d 01 01`, but user observed Pro Controller toast and Joy-Con registration did not complete. pytest timeout was a probe wrapper issue caused by responder replacement |
| hardware Joy-Con L SDP policy retest, `build\hardware\joycon-left-sdp-policy-20260706` | hardware-pass | 1 passed in 24.40s。trace は `local_bluetooth_address_configured address=001bdcf99f7d`、`device_info_bluetooth_address_configured address=001bdcf99f7d`、`device_info_data=04000102001bdcf99f7d0101` を記録し、responder wrapper 経由の hardware test が timeout せず完了した |
...
- Joy-Con-specific SDP / descriptor-adjacent values は unit_036 で SDP policy の範囲だけ実装した。HID descriptor bytes と Class of Device は未変更。
- 同じ物理 dongle address を Pro / Joy-Con で使い回した場合の Switch 側登録名の扱いは未検証。今回の成功観測を cross-console / cross-firmware guarantee にしない。
