...
| Bumble / transport | required | done for source, unit tests, and hardware | Bumble 0.0.230 の `disconnect_interrupt_channel()`、`disconnect_control_channel()`、`on_l2cap_channel_close()` を source fact として確認し、`BumbleHidTransport.request_disconnect()` と L2CAP close bridge を unit test で固定した。2026-07-02 の修正前 hardware run では Switch 実機で L2CAP close、`disconnect_request status=requested`、`disconnect_request_terminal status=closed` まで観測したが、`transport_close_complete` が欠落した。race 修正後の rerun では `connected`、neutral `0x30`、L2CAP close、`disconnect_request status=requested`、`disconnect_request_terminal status=closed`、`transport_close_complete` を観測した。後続の visibility 調査で Bumble 0.0.230 `Device.power_off()` が Classic scan を明示停止しないことを source fact として確認し、`BumbleHidTransport.close()` が `set_discoverable(False)` / `set_connectable(False)` を呼ぶことを unit test と hardware で固定した。Bumble 0.0.230 の incoming Classic connection handler は host listener 登録済みの `Device.on_connection_request()` から deprecated `host.send_command_sync()` を呼ぶため、`BumbleHidTransport` 側で listener を diagnostics bridge に差し替え、upstream handler 実行中だけ `AsyncRunner.spawn(host.send_async_command(...))` へ写像することを unit test と `-W error` hardware run で固定した |
| OS / driver / adapter | required | observed | Windows / CSR8510 A10 / WinUSB / `usb:0` で、修正後の connected close ordering と full observed handshake 後の A exit / close path を観測した。artifact は `.pytest_cache\hardware\unit_014\20260702-211502-close-disconnect-connectivity\close-disconnect.jsonl`、`.pytest_cache\hardware\unit_014\20260702-220940-close-disconnect-after-scan-cleanup\close-disconnect.jsonl`、`.pytest_cache\hardware\unit_014\20260702-warning-fix-listener\post-handshake-a-close.jsonl` |
...
| Bumble 0.0.230 `Device.host` setter は `Device.on_connection_request()` を host の `connection_request` listener として登録する | source fact | `.venv/Lib/site-packages/bumble/device.py:2610` | local package source |
| Bumble 0.0.230 `Device.on_connection_request()` は incoming Classic connection accept/reject で deprecated `Host.send_command_sync()` を呼ぶ | source fact | `.venv/Lib/site-packages/bumble/device.py:6096`, `.venv/Lib/site-packages/bumble/host.py:836` | local package source |
| `BumbleHidTransport` は host `connection_request` listener を diagnostics bridge に差し替え、upstream handler 実行中だけ deprecated sync helper を `AsyncRunner.spawn(host.send_async_command(...))` に写像する | implementation fact / hardware observation | `src/swbt/transport/bumble.py`, `tests/unit/test_bumble_transport.py`, `.pytest_cache/hardware/unit_014/20260702-warning-fix-listener/pytest-debug.log` | current |
...
| done | incoming Classic connection request bridge が Bumble の deprecated sync command warning を出さない | regression | unit / hardware | yes | `test_bumble_connection_request_bridge_avoids_deprecated_sync_command_warning` で host listener 差し替えを固定。`-W error` 付き hardware run で full observed handshake、A exit、neutral、close まで pass |
...
| `src/swbt/transport/bumble.py` | modify | Bumble channel disconnect helper bridge、incoming Classic connection request listener bridge |
...
| `uv run pytest tests\unit\test_bumble_transport.py::test_bumble_connection_request_bridge_avoids_deprecated_sync_command_warning -q` | red then pass | 実機 `-W error` で出た `DeprecationWarning: Use utils.AsyncRunner.spawn() instead.` を host event 経由の fake で再現。host listener 差し替え後は warning-as-error で pass |
| `uv run pytest tests\unit\test_bumble_transport.py -q -W error` | pass | 24 passed in 0.26s。incoming Classic connection request bridge の warning regression 追加後 |
