...
## 2026-07-04: Linux / macOS adapter 対応の experimental 扱い

### 現状

public docs では Windows 11、CSR8510 A10、WinUSB / libwdi、Switch 2 firmware 22.1.0 の組み合わせだけを confirmed behavior として扱っている。
Linux / macOS の USB Bluetooth dongle 準備は Bumble upstream docs に手順があるが、この repo では adapter open、pairing、reconnect、入力反映をまだ確認していない。

### 観察

Bumble upstream docs では、USB transport は `libusb-1.0` を使い、Linux では kernel / BlueZ が dongle を掴む場合の `hciX down` と USB device permission、macOS では外付け HCI を macOS Bluetooth stack に掴ませないための `bluetoothHostControllerSwitchBehavior="never"` が示されている。
これらは `swbt-python` の実機接続に必要な OS 準備の候補だが、Switch-facing pairing と HID report loop まで動く根拠にはならない。

### 方針

Linux / macOS は unsupported と切り捨てるのではなく、experimental target として実装単位を立てる。
最初の完了条件は、OS 準備手順の文書化、Bumble adapter open、Switch-facing 動作を含まない smoke test、失敗時 diagnostics の記録までに絞る。
pairing、reconnect、入力反映は後続の hardware-gated item に分け、確認できるまで supported 扱いにしない。
