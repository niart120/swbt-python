...
| fail | 実機で `0x01` output report を受信できる | new | hardware | yes | 2026-07-02 の Switch 側接続状態リセット後も L2CAP open 後に `output_report_rx` 未観測のまま Switch 側が reason 19 で切断 |
| fail | 観測された subcommand sequence が trace に残る | new | hardware | yes | `subcommand_rx` 未観測。post-fix / SDP service-name / reset-state failure trace と no-report-window diagnostic は `docs/hardware-test-log.md` に記録 |
