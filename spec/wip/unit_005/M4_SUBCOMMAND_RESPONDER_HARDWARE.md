...
| green | 実機で `0x01` output report を受信できる | new | hardware | yes | 2026-07-02 の link-policy-only run で `output_report_rx` report `0x01` / subcommand `0x02` を観測 |
| green | 観測された subcommand sequence が trace に残る | new | hardware | yes | 2026-07-02 の link-policy-only run で `output_report_rx`、`subcommand_rx`、`subcommand_reply_tx`、`report_tx` reason `subcommand_reply` を記録 |
| observed | 観測 subcommand に `0x21` reply を返せる | new | hardware | yes | 2026-07-02 の link-policy-only run で `0x02` device info に `0x21` reply を返した。全主要 subcommand の実機網羅ではない |
| observed | 未対応 subcommand があれば docs に反映されている | characterization | hardware | yes | 未対応 subcommand は未観測。link-policy run でも `unsupported_subcommand` は出ていない |
...
| `uv run pytest tests\unit\test_bumble_transport.py tests\unit\test_source_audit_fixtures.py -q` | pass | 23 passed。Classic default link policy `0x0005`、HIDP DATA header、SET_REPORT callback、HID SDP service name / language base / policy、source-audit fixture を確認した |
...
| `uv run pytest tests\unit -q` | pass | 93 passed。最小実装へ戻した後、profile default は元の `0x91` / `0x00`、Bumble transport 変更は Classic default link policy `0x0005` のみに整理して確認した |
