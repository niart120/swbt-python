...
| red | hardware test が configured local address と Device Info address の一致を要求する | new | hardware | yes | 2026-07-06 の初回 retest は address 注入 timing が早すぎて fail。`034000` run は Device Info address を送ったが計測 wrapper が外れて timeout |
...
| hardware Joy-Con L retest after in-place responder update | pending | 実機再実行は未承認。次は SDP / descriptor-adjacent 値の切り分け後に実行する |
...
- Device Info address を正しく送っても Pro Controller toast が残ったため、identity source は Device Info 以外も調べる。
