...
| todo | Direct input と subcommand reply が共通 timer / send lock を通り、prefix と送信順が一致する | new | unit | no | 共通 sender の決定的な競合 test を置く |
| todo | Direct の `close(neutral=True/False)` が trailing neutral の有無、commit、cleanup を契約どおり処理する | new | integration | no | 追加 input report 数を比較する |
| todo | Direct Pro / Joy-Con L / Joy-Con R が同じ runtime と profile validation を共有する | new | integration | no | 3 profile の正常入力と unsupported input を確認する |
