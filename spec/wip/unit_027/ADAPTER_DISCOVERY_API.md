...
| todo | `swbt-probe adapters --json` は `list_adapters()` の結果を `adapters` 配列に出し、`opens_adapter=false` を維持する | regression | unit | no | `candidate_adapters` は出さない。VID/PID は int と 4 桁大文字 hex を併記する |
| todo | `swbt-probe adapters --json` は discovery failure を adapter open 失敗として報告しない | edge | unit | no | exit code `1`、`status="discovery_error"`、`error.type="AdapterDiscoveryError"` を返す |
...
- 現行 `swbt-probe adapters --json` は実 USB 列挙をせず、固定候補 `usb:0` と環境情報を返している。API 実装後は `candidate_adapters` を維持せず、`adapters` 配列へ置き換える。`opens_adapter=false` は no-open 契約として維持する。
...
- `list_adapters()` の結果を `SwitchGamepad(adapter=...)` で open できるかの characterization。
