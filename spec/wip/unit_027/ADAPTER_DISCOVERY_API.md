...
API 名は仮に `list_adapters()`、戻り値の要素は `AdapterInfo` とする。この API が列挙するのは PC 側の専用 USB Bluetooth dongle 候補であり、Nintendo Switch 本体や周辺の Bluetooth host を探索する API ではない。

この unit では仕様を作成するだけで、実装は行わない。
...
| 既存 CLI no-open | `_run_adapters()` は `opens_adapter=false` と `candidate_adapters=["usb:0"]` を返し、`SwitchGamepad` を作らない | implementation fact | `src/swbt/probe.py` | done |
| 既存 CLI regression | `test_swbt_probe_adapters_json_reports_no_open_environment` は `opens_adapter=false` と `candidate_adapters` を確認する | implementation fact | `tests/unit/test_probe_cli.py` | done |
...
- `usb1.USBContext()` と descriptor getter が各 OS でどの程度の権限を要求するかは、この unit では実機確認しない。
...
| CLI JSON | `swbt-probe adapters --json` | `opens_adapter=false` と adapters 配列を出す | 既存 `candidate_adapters` は互換維持が必要なら `AdapterInfo.name` から生成する |
...
`vendor_id` と `product_id` は Python API では `int` とする。CLI JSON では、人間が照合しやすいように 4 桁大文字 hex 文字列を追加するか、`vendor_id` / `product_id` を int のままにするかを実装時に決める。どちらを選ぶ場合も unit test で固定する。
...
|---|---|---|---|---|---|
| todo | `list_adapters()` と `AdapterInfo` を `swbt` module root から import できる | new | unit | no | Bumble object 型を公開しない |
| todo | `AdapterDiscoveryError` を `swbt` module root から import でき、`SwbtError` の派生として扱える | new | unit | no | no-open discovery の public 例外として固定する |
| todo | fake USB enumerator が 1 件の Bluetooth HCI device を返すと、`list_adapters()` が `AdapterInfo(name="usb:0")` を返す | new | unit | no | 実USB列挙を使わない |
| todo | fake USB enumerator の HCI device から VID/PID、manufacturer、product、serial、bus/device 情報を `AdapterInfo` へ写す | new | unit | no | descriptor getter は fake で観測する |
| todo | 同じ VID/PID の HCI device が複数ある場合、`aliases` に duplicate index または serial alias が入る | edge | unit | no | `usb_probe.py` の重複 moniker 生成を仕様化する |
| todo | serial、manufacturer、product の取得だけが失敗しても、該当 field を `None` にして候補を返す | edge | unit | no | descriptor access 失敗を列挙失敗にしない |
| todo | HCI class ではない USB device は default の `list_adapters()` に含めない | new | unit | no | forced mode や unknown USB device は初期対象外 |
| todo | HCI 候補が 0 件の場合、`list_adapters()` は空 tuple を返す | edge | unit | no | no devices と discovery failure を分ける |
| todo | libusb load または USBContext 初期化が失敗した場合、`AdapterDiscoveryError` を投げる | edge | unit | no | `TransportOpenError` にしない |
| todo | `list_adapters()` は `open_transport()`、`open_usb_transport()`、`device.open()`、`SwitchGamepad` を呼ばない | regression | unit | no | monkeypatch / spy で no-open 契約を固定する |
| todo | `swbt-probe adapters --json` は `list_adapters()` の結果を JSON に出し、`opens_adapter=false` を維持する | regression | unit | no | CLI 互換として `candidate_adapters` を残すかは実装時に固定 |
| todo | `swbt-probe adapters --json` は discovery failure を adapter open 失敗として報告しない | edge | unit | no | JSON 形状と exit code を実装時に決めて test 化する |
| todo | `swbt-probe adapters` の human output が no-open 境界と候補 adapter name を表示する | regression | unit | no | help text の no-open 文言も維持する |
| todo | public API docs と docstring が `AdapterDiscoveryError` を候補 0 件ではなく列挙不能の例外として説明する | new | unit | no | `docs/api.md` と public docstring test で固定する |
| deferred | Windows で `uv run swbt-probe adapters --json` を実 USB dongle ありで実行する | characterization | bumble | yes | 明示承認、対象 adapter、command、cleanup plan が必要。adapter open はしない |
...
| deferred | `list_adapters()` で返った adapter を `SwitchGamepad(adapter=...)` で open できるか確認する | characterization | bumble | yes | この unit の必須条件にしない。adapter open 承認が別途必要 |
...
- 現行 `swbt-probe adapters --json` は実 USB 列挙をせず、固定候補 `usb:0` と環境情報を返している。API 実装後も `opens_adapter=false` は維持する。
- `usb:N` は短く便利だが、接続順や他の dongle に影響される。利用者向け docs では、同じ VID/PID が複数ある場合に serial alias を優先する判断を書けるとよい。
...
### 10.1 この仕様作成で編集するファイル

| path | change | 内容 |
|---|---|---|
| `spec/wip/unit_027/ADAPTER_DISCOVERY_API.md` | new | この作業仕様 |

### 10.2 後続実装で変更候補になるファイル

| path | change | 内容 |
|---|---|---|
| `src/swbt/__init__.py` | modify | `list_adapters`、`AdapterInfo`、`AdapterDiscoveryError` を公開する |
| `src/swbt/adapter_discovery.py` | new | no-open adapter 列挙 API の実装候補 |
...
| `spec/initial/api.md` | modify | API が固まった後に正本へ反映する候補 |
| `spec/initial/transport-bumble.md` | modify | adapter discovery と adapter open 境界を追記する候補 |
| `docs/hardware.md` | modify | `list_adapters()` と `swbt-probe adapters` の使い分けを追記する候補 |

## 11. 検証

| command | result | notes |
|---|---|---|
| `uv run pytest tests/unit` | pending | この turn は仕様書作成のみで実装を行わないため、後続 TDD で実行する |
| `uv run pytest tests/integration` | pending | この turn は仕様書作成のみで実装を行わないため、後続 TDD で必要になった時点で実行する |
| `uv run pytest -m bumble` | pending-approval | 実 USB device の列挙や adapter open へ進む確認は、対象 adapter と cleanup plan を決めてから承認を取る |
| `uv run pytest -m hardware` | pending-approval | Switch-facing 動作をこの unit の必須条件にしない。実行する場合は別途承認を取る |
| `uv run swbt-probe adapters --json` | pending | 既存 CLI の実行も実 USB 環境に依存し得るため、この仕様作成 turn では実行しない |

## 12. 実機実行条件

| 項目 | 内容 |
|---|---|
| 実機要否 | Python 実装の単体テストでは不要。実 USB 列挙の characterization では必要 |
| 承認範囲 | この spec-only turn ではなし。後続で実 USB 列挙を行う場合は、対象 OS、対象 dongle、実行 command、adapter open なし、cleanup plan を明示して承認を取る |
| adapter | 未使用。文書例として `usb:0` を使う場合も固定値扱いしない |
| 実行遮断 | 環境変数による遮断は採用しない。明示承認、対象 adapter、command、cleanup plan で管理する |
| log / artifact | 実 USB 列挙を行った場合は OS、driver、dongle identity、Bumble version、Python version、command、result、cleanup を `spec/hardware-test-log.md` に記録する |
| cleanup | `list_adapters()` / `swbt-probe adapters` の実 USB 列挙だけなら adapter open、advertising、pairing、report loop は発生しない。adapter open へ進む確認は別承認にする |

## 13. 先送り事項

- `AdapterInfo` の JSON 表現で VID/PID を int にするか hex string を併記するか。
- `AdapterDiscoveryError` の field 設計。最低限、元例外型、platform、Bumble version、libusb load 状態を診断に残せるとよい。
- non-HCI class の USB device を `include_unknown=True` で返す必要があるか。
- `usb:N` ではなく serial alias を docs で推奨する条件。
- 実 USB dongle ありの Windows / macOS / Linux 列挙結果。
- `list_adapters()` の結果を `docs/hardware.md` の troubleshooting にどう載せるか。
