...
| 項目 | 要否 | 状態 | 根拠 / 理由 |
|---|---|---|---|
| Switch HID / report bytes | required | partial | incoming trace では subcommand sequence まで観測したが、active reconnect は HID control / interrupt channel open 前に切断された。Switch UI の起動条件が未記録だったため、HOME / 通常画面条件の active reconnect 結果としては扱わない |
| Bumble / transport | required | done-for-M6 | Bumble 0.0.230 は `DeviceConfiguration.keystore` に `JsonKeyStore:<filename>` を渡すと `Device.power_on()` 時に `KeyStore.create_for_device()` 経由で `JsonKeyStore` を作る。`JsonKeyStore.get_all()` は peer address と key object の組を返す。active Classic connection は `Device.connect(..., transport=PhysicalTransport.BR_EDR, timeout=...)` / `connect_classic()` 経由で開始できる。`Device.update_keys()` は keystore 例外を内部で捕捉するため、swbt-python は keystore wrapper で `update()` 成否を diagnostics に記録する。link key request の詳細は未監査だが、M6 では key store 書き込み、peer discovery、active attempt、failure diagnostics まで固定した |
| OS / driver / adapter | required | needs-rerun | 2026-07-03 の `usb:0` / CSR8510 A10 / WinUSB run は active reconnect timeout と incoming trace 分離を記録した。ただし Switch UI 起動条件が未記録だったため、起動条件を trace に残す修正版 hardware test で再実行する |
...
| needs-rerun | key store ありで active bond reuse reconnect 成功 / 失敗を実機で記録する | new | hardware | yes | 2026-07-03 run は active attempt と timeout を記録したが、Switch UI 起動条件が未記録だったため HOME / 通常画面条件で再実行する |
| needs-rerun | active reconnect run で新規 pairing event の有無、authentication、encryption、HID channel open を記録する | characterization | hardware | yes | 2026-07-03 run の active trace では `classic_pairing` と `key_store_update` は出ず、HID channel open 前に reason 19 で切断された。起動条件未記録のため再実行する |
| needs-rerun | incoming bond reuse を実機で観測する場合は Switch 側操作の有無を記録する | characterization | hardware | yes | 2026-07-03 run は incoming route 分離を記録したが、`classic_pairing` と `key_store_update` も出たため pairing-free bond reuse とは扱わない |
| needs-rerun | hardware matrix に reconnect 結果が反映される | new | hardware | yes | matrix は現時点で inconclusive。起動条件記録付き rerun 後に更新する |
...
| observed-partial | `pairing_complete` / `connection_authentication` diagnostics が実機 trace に出るか確認する | characterization | hardware | yes | 2026-07-03 run でも `link_key_available` と `connection_encryption_change` は出たが、`pairing_complete` と `connection_authentication` は記録されなかった |
...
| `uv run pytest tests\hardware\test_reconnect_keystore.py -m hardware ...` | pending-approval | operator instruction を trace に残す修正版で再実行する。初回 pairing は controller search / change grip order、active reconnect は HOME / 通常画面で待機する |
...
- [ ] 実機 reconnect 観測は起動条件、承認、command、cleanup、結果を `docs/hardware-test-log.md` に記録した
