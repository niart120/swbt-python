...
|---|---|---|---|---|---|
| todo | root export が target controller API を表す | new | unit | no | `swbt.__all__` の target test。M2 まで `xfail` 可 |
| todo | `SwitchGamepad` は abstract で直接生成できない | new | unit | no | `inspect.isabstract()` と direct instantiate failure |
| todo | public concrete controller が共通 interface を実装する | new | unit | no | `issubclass(ProController, SwitchGamepad)` など |
| todo | public concrete constructor が internal seam を受け取らない | new | unit | no | `profile`, `device_name`, `transport` 不在 |
| todo | breaking change 方針が docs / spec に残っている | regression | unit / docs | no | custom transport public extension point の削除を曖昧にしない |
