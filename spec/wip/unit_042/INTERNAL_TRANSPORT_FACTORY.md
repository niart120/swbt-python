...
| todo | `HidDeviceTransport` が root export されない | new | unit | no | package import test |
...
| todo | default transport factory が Bumble import 遅延を維持する | regression | unit | no | import boundary |
...
| path | change | 内容 |
|---|---|---|
| `src/swbt/gamepad/_transport_factory.py` | add / modify | internal transport factories |
| `src/swbt/gamepad/_config.py` | modify | runtime builder が factory を受け取る |
| `src/swbt/gamepad/controllers.py` | modify | public constructor から `transport` 削除 |
| `src/swbt/__init__.py` | modify | `HidDeviceTransport` export removal |
| `tests/helpers/` | add / modify | fake transport helper 候補 |
| `src/swbt/_testing/` | add / modify | repo 外 integration test が必要な場合の候補 |
...
| `uv run pytest tests/unit -q` | pass | `349 passed, 1 xfailed`。残る xfail は root `HidDeviceTransport` export |
| `uv run pytest tests\integration\test_switch_gamepad_fake_transport.py -q` | pass | `91 passed`。fake transport integration は `swbt._testing.gamepad` helper 経由で実行 |
| `uv run pytest tests\integration\test_examples.py -q` | pass | `2 passed`。example integration は internal testing helper 経由で fake transport を注入 |
| `uv run ruff check tests\integration\test_switch_gamepad_fake_transport.py tests\integration\test_examples.py src\swbt\_testing` | pass | `All checks passed!` |
