...
| green | `wait_connected()` が fake connected callback を待って完了する | new | integration | no | `test_wait_connected_completes_after_fake_connected_callback` で固定。timeout path は未固定 |
| todo | `tap(Button.A)` が A 押下 report と release report を fake transport に残す | new | integration | no | M0 report builder を使用 |
| todo | `press(Button.L, Button.R)` 後の periodic report が L+R を含む | new | integration | no | state store 経由 |
| todo | `release(Button.L, Button.R)` 後の report が該当 button を clear する | new | integration | no | 解除の contract |
| todo | output report 注入から `0x21` reply が送信される | new | integration | no | fake transport receive callback |
| todo | reply queue に `0x21` がある場合、次送信は `0x30` ではなく `0x21` になる | new | integration | no | 優先制御の中核 |
| todo | `close(neutral=True)` が trailing neutral report を記録する | new | integration | no | close 時の fail-safe |
| todo | 複数 task から `press()` / `release()` しても state が破壊されない | edge | integration | no | lock の効果を観測 |
| todo | callback 例外が diagnostics に記録され、`close()` で後始末できる | edge | integration | no | lifecycle failed path |
| todo | public API が Bumble 型を公開していない | regression | unit | no | import 境界 |
...
| `uv run pytest tests\unit tests\integration -q` | pass | 68 passed。unit と fake transport integration を確認した |
| `uv run ruff format --check .` | pass | 25 files already formatted |
