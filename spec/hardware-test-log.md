...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Windows | CSR8510 A10 | WinUSB / libwdi 6.1.7600.16385 | `usb:0` | Switch 2 | 22.1.0 | observed-pass | pass | pass for full observed M5 handshake sequence; unit_028 body/buttons/controller grip color SPI reply pass | observed-pass for Button A / neutral / D-pad / left and right stick | observed-pass for active bond reuse reconnect after explicit Classic authentication/encryption | 2026-07-04 unit_013 input semantics, unit_007 active reconnect, and 2026-07-05 unit_028 controller color SPI reply runs | 2026-07-05 | Release gate minimum は pairing、L2CAP、subcommand 応答、Button A、neutral。unit_013 で Switch 2 / firmware 22.1.0 の D-pad と left / right stick 反映も確認済み。unit_028 で Switch からの `0x6050` SPI read に custom body/buttons/grip color bytes が返ることを trace で確認した。ユーザは Switch UI で緑 body / 黄 buttons の controller 表示を目視確認したが、左右 grip は 15 秒 hold と 30 秒 hold のどちらでも custom 青/赤に見えず緑のままと報告した。L+R 同時押し表示は Switch button check UI の表示制約として扱う。incoming run は route 分離と subcommand sequence を記録したが、`classic_pairing` と `key_store_update` も出たため pairing-free incoming bond reuse とは扱わない |
