...
- CSR BD_ADDR local-address Switch pairing: 2026-07-20 に専用 `usb:0` の volatile address を `02:1B:DC:F9:9F:7D` へ変更し、二段階 address guard 付き pairing probe を実行した。standard HCI / CSR default-store は local address で一致し、Bumble `power_on()` 後も同 address、advertising、connection request、Classic pairing、fresh key store 保存、HID connected、neutral `0x30` 1件、disconnect、adapter close を確認した。local address が対象 Switch に受理されることは observed-pass。元 address と別登録に見えたかはユーザが目視確認を逃したため unobserved
