...
- cleanup: run reached `transport_close_complete` and `manual_joycon_profile_cleanup connection_state=closed`. It sent neutral after SR+SL before close. The adapter was released.
- notes: `0x22` is NFC/IR MCU state. The follow-up code change ACKs source-audited argument bytes `0x00`, `0x01`, and `0x02` with `0x80 0x22` and no data. It does not model NFC/IR MCU state. R1 hardware rerun after that change is still pending.
