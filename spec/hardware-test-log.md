...
- 2026-07-27 unit_087 Python 3.12 experimental clock比較と採否: Windows `Windows-11-10.0.26200-SP0` / dedicated `usb:0` / CSR8510 A10 `0A12:0001` / WinUSB / Bumble 0.0.233 / Python 3.12.10 / swbt-python 0.5.4 (`e4b6b4e`と実験差分)で、`ReportSender`と`ReportLoop`の既定clockを試験的に`perf_counter_ns`へ変更し、既存profileによるactive reconnect後に正Z yaw `1.0 rad/s`を送信した。Switch model / firmwareとdriver assignmentはこのrunでは再確認していない。Directを8 ms → 16 ms → 8 msの各8秒で実行すると、連続IMU block一致は全区間0件になったが、8 ms区間のACL dispatchは908 / 897件、2 ms未満gapは435 / 424件で、利用者はカクつきを目視した。credit待ちは最大0であり、clock修正だけではDirect呼び出し側のPython 3.12 `asyncio` schedulingは改善しない。同じDirect probeでevent loop clockも診断用に`perf_counter`へ差し替えると、ACL reportは1000 / 501 / 1001件、8 ms gap平均8.0082 / 8.0009 ms、2 ms未満gap 0件、連続IMU block一致0件となり、利用者は回転が滑らかになったと目視した。Periodicはevent loopを差し替えず、実験的clock修正だけで8 ms × 8秒を実行し、ACL report 998件、gap平均8.0081 ms（min 6.0196、max 15.6433）、2 ms未満gap 0件、連続IMU block一致0件、credit待ち最大0となり、利用者は滑らかな回転を目視した。全runでprofile不変、event drop 0、cleanup完了を確認した。Bumble stderrの`no pending response future to set`は各runで1件出たが、processは終了コード0。承認範囲は`usb:0` open、既存profile active reconnect、HID control / interrupt、Direct / Periodic正Z yaw、neutral close、adapter releaseで、新規pairing、button / stick、BD_ADDR書換えは実行していない。最終判断では、このclock修正とevent loop差し替えを製品へ採用せず、最低Pythonバージョンを3.13へ引き上げる。artifactは`tmp/hardware/issue_152/credit-gyro-20260727/py312-perf-clock-direct/`、`py312-perf-clock-highres-loop-direct/`、`py312-perf-clock-periodic-8ms/`。

- 2026-07-27 Issue #152 Python / Bumble matrix and quaternion payload characterization: 同じ fresh-pairing profile、Switch 画面、正 Z yaw `1.0 rad/s`、8 ms → 16 ms → 8 ms の条件で比較した。Python 3.13.5 / Bumble 0.0.230 は利用者目視でカクつきなし。Python 3.12.10 / Bumble 0.0.233 と Python 3.12.10 / Bumble 0.0.230 は、どちらも8 ms区間でカクつき、16 ms区間だけ安定して見えた。したがって、この実験ではBumble 0.0.230 / 0.0.233の差は症状を支配していない。Python 3.12.10 / Bumble 0.0.233でasyncio loop clockだけを`perf_counter`へ差し替えると、ACL dispatchは8 ms区間で2 ms未満gap 0件、Bumble queue待機packet最大0となった。USB sink enqueue、OUT transfer slot取得、libusb transfer callbackも約8 ms間隔で、USB queue待機packet最大1だった。それでも利用者目視のカクつきは変わらず、16 ms区間だけ安定した。この構成ではHCI credit不足、Bumble `DataPacketQueue`待機、USB OUT transfer待機のいずれも、カクつきの必要条件ではない。実機を使わない追加characterizationでは、`FakeHidTransport`へ8 ms間隔で251件のquaternion IMU reportを生成した。Python 3.12.10は`time.monotonic_ns()`が`GetTickCount64()`、公称分解能15.625 msで、250区間中122区間が経過0 nsとなり、同じ122区間で連続する36-byte IMU blockが完全一致した。残る128区間は`monotonic_ns()`が15 msまたは16 ms進み、IMU blockも更新された。Python 3.13.5は`QueryPerformanceCounter()`、公称分解能100 nsで、経過0 nsと連続IMU block一致はともに0件だった。実装上、`ReportSender`は既定clockとして`time.monotonic_ns`を使い、quaternion modeは前reportとの差分時間で姿勢を積分するため、Python 3.12では8 ms入力が「姿勢更新なし」と「15–16 ms分の姿勢更新」を交互に含む。asyncio loop clockだけを差し替えても`ReportSender`の既定clockは変わらない。これらのimplementation fact、characterization、hardware observationは、Python 3.12でのquaternion payload時間量子化を今回のカクつきの主因とする説明に一致する。Switch model / firmwareはこのrunでは再取得しておらず、別OS、別Python patch version、別Switch firmwareへの一般化は未検証。artifactは`tmp/hardware/issue_152/credit-gyro-20260727/py313-bumble0230/`、`py312-bumble0233/`、`py312-bumble0230/`、`py312-bumble0233-highres-loop-rerun/`、`py312-highres-usb-probe/`。

- 2026-07-27 Issue #152 Direct gyro / HCI credit A-B-A: Windows `Windows-11-10.0.26200-SP0` / dedicated `usb:0` / CSR8510 A10 `0A12:0001` / WinUSB service / libwdi `6.1.7600.16385` / Bumble 0.0.233 / Python 3.13.5 / swbt-python 0.5.4 (`e4b6b4e`) で実行した。Switch model / firmware はこの run では再取得していない。利用者が Switch 側の接続情報を削除し、「持ちかた／順番を変える」画面を準備した後、`test_switch_adapter_default_profile_fresh_pairing_and_close` を実行して `1 passed in 4.83s`。`classic_pairing`、`key_store_update(status=succeeded)`、`protocol_ready(route=pairing)`、neutral input、`transport_close_complete` を確認し、同じ profile を Direct 計測へ使用した。計測は Project_Demi の診断キー `J` と同じ正 Z yaw `1.0 rad/s`、静止時加速度 Z `1 G` を、固定 deadline で 8 ms × 8秒、16 ms × 8秒、8 ms × 8秒の順に送信した。Direct send / ACL dispatch は各区間 1,000 / 1,000件、500 / 500件、1,001 / 1,001件。ACL dispatch gap は順に mean 7.999949 ms（min 6.1956、max 10.0396）、mean 16.001558 ms（min 13.9599、max 18.0976）、mean 8.001519 ms（min 6.4098、max 9.6325）で、8 ms両区間の2 ms未満と12–20 ms gapはいずれも0件、report timer不連続も0件だった。Bumble `DataPacketQueue` は `max_in_flight=10` に対して実測最大 in-flight 4、enqueue後の待機packet最大0。全体でACL enqueue 2,530件、dispatch 2,530件、completion 2,530件で、各 `HCI Number Of Completed Packets` のpacket countはすべて1だった。利用者は3区間を通して周期的なカクつきは見えなかったと観測した。この構成ではIssue #152のBumble queue待機と近接送出を再現していない。終了時はprofile不変、trace `error` 0件、connection state `closed`、`transport_close_complete`を確認した。HCI disconnect requestは0.25秒でtimeoutし、Bumble stderrに`no pending response future to set`が1件出たが、adapterはpostflightのno-open列挙で再検出できた。承認範囲はfresh pairing、HID advertising、初期化応答、Direct正yaw、neutral close、adapter release。artifactは`tmp/hardware/issue_152/credit-gyro-20260727/`のprofile、fresh-pairing trace、`hci-credit-gyro-trace.jsonl`、`hci-credit-gyro-events.json`。
...
- 2026-07-25 unit_081 protocol handshake separation: Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 / swbt-python 0.5.1 で、専用adapterのread-only preflightを実行した。HCI / CSRの双方で `0E:08:71:C0:B4:5C`、CSR status `0`、一致、adapter closeを確認した。続いてPro fresh pairing、Pro active reconnect、Direct Pro active reconnect、Joy-Con L/R fresh pairingを実行し、pytestは `4 passed, 1 failed in 27.33s`。Pro fresh / Periodic reconnect、Joy-Con L fresh、Joy-Con R freshは protocol ready、handshake bootstrap、supported report mode後のneutral、reply、neutral close、transport close completeまでpassした。Direct Pro初回runもprotocol ready前に`handshake_report_mode`のneutral 3件、ready後1秒の自動report 0件、終了時direct neutral 1件、transport close completeをtraceで確認したが、hardware assertionが旧`reason="periodic"`だけを探索していたため失敗した。assertionを新しい`handshake_bootstrap` / `handshake_report_mode` / `periodic`の自動送信理由へ更新後、ユーザ承認下でDirect Pro reconnectを2回再実行したところ、いずれもSwitch側認証で `HCI AUTHENTICATION_FAILURE_ERROR [0x05]` が発生し、protocol handshake前に `active_reconnect_result(status="failed")` となった。追加reconnectやfresh pairingは行わず停止した。artifactは `tmp/hardware/unit_081/protocol-handshake-20260725/`。approvalは会話上で専用`usb:0`、advertising、pairing、HID subcommand handling、neutral report loop、neutral close、adapter releaseを明示承認。non-neutral inputとBD_ADDR書換えは未実行。

- 2026-07-25 unit_075 happy-case Button A close: Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 / swbt-python 0.5.1 で、`tests/hardware/test_close_disconnect.py::test_switch_close_after_full_handshake_and_a_exit_for_manual_ui_confirmation` を実行し `1 passed in 10.15s`。fresh pairing、full handshake、Button A送信、neutral復帰、close開始、disconnect request、disconnect terminal `closed`、transport close completeの順序を確認した。Switch UIでButton Aによる登録画面からの画面遷移を利用者が目視確認した。artifactは `tmp/hardware/unit_075/happy-20260725-pro-button-a/post-handshake-a-close.jsonl`。approvalは会話上でButton A、`usb:0`、pairing、neutral close、adapter closeを明示承認。

- 2026-07-25 unit_075 happy-case subcommand observation: Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 / swbt-python 0.5.1 で、`tests/hardware/test_pairing_l2cap.py::test_switch_subcommand_observation_window_replies_to_all_observed_commands` を実行し `1 passed in 14.64s`。fresh pairing後に5秒以上の観測窓を持ち、`subcommand_rx` 16件すべてに `subcommand_reply_tx` 16件、`0x21` reportを確認した。`protocol_ready` 1件、`unsupported_subcommand` 0件、`error` 0件、disconnect terminal `closed`、transport close complete。artifactは `tmp/hardware/unit_075/happy-20260725-pro-subcommands/subcommand-observation-window.jsonl`。Switch UIの追加目視判定はこのtestでは行わず、neutral closeとadapter closeを完了した。approvalは会話上でsubcommand observation、`usb:0`、pairing、neutral close、adapter closeを明示承認。

- 2026-07-25 unit_075 happy-case fresh pairing: Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 / swbt-python 0.5.1 で、`tests/hardware/test_pairing_profile.py::test_switch_adapter_default_profile_fresh_pairing_and_close` を実行し `1 passed in 5.23s`。adapter-default Pro profileを新規作成し、adapter address `0E:08:71:C0:B4:5C`、`classic_pairing`、key store update succeeded、control / interrupt L2CAP、protocol ready、`0x21` reply、neutral report、disconnect terminal `closed`、transport close completeを確認した。local address rewriteは行っていない。artifactは `tmp/hardware/unit_075/happy-20260725-pro-fresh/pairing-profile-adapter-default-pro.json` と `pairing-profile-adapter-default-fresh-pairing.jsonl`。Switch UIで利用者が登録表示を目視確認した。approvalは会話上でfresh pairing、`usb:0`、neutral close、adapter closeを明示承認。

- 2026-07-25 unit_075 happy-case reconnect: Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 / swbt-python 0.5.1 で、既存 adapter-default Pro profile を使う `tests/hardware/test_pairing_profile.py::test_switch_adapter_default_profile_reuses_address_after_normal_close` を実行した。adapter address `0E:08:71:C0:B4:5C` は profile namespace と一致し、Bumble HID初期化、bonded peer選択、active reconnect開始まで passしたが、Switch側認証で `HCI AUTHENTICATION_FAILURE_ERROR [0x5]` が発生し `active_reconnect_result(status="failed", failure_reason="transport_error")` となった。pairing / advertising / key-store update は実行していない。artifact は `tmp/hardware/unit_075/happy-20260725-pro-reconnect/pairing-profile-adapter-default-reconnect.jsonl`。テストの `finally` で neutral close、disconnect request unavailable、transport close complete、adapter releaseまで完了。stored link key と現在の bond不一致の可能性があるため、fresh pairingなしの再試行は停止した。approval は会話上で `usb:0` の既存profile active reconnect 1本を明示承認。

- unit_069 controller handshake readiness: 2026-07-24 に Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 で実行。Proはready trace assertion付きactive reconnect `1 passed in 4.98s`、fresh pairing `1 passed in 5.39s`、Direct neutral-only reconnect `1 passed in 5.44s`。Joy-Con Lは追加SR+SLなしでfresh `1 passed in 5.15s`、reconnect `1 passed in 5.94s`。Joy-Con Rも追加SR+SLなしでfresh `1 passed in 5.52s`、reconnect `1 passed in 6.46s`。全runでbootstrapを最初のsubcommandで停止し、`0x03 30`とnonzero player lights reply後にready。cleanup完了
- unit_068 Direct close 前入力反映: 2026-07-24 に Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 で Direct Pro / Button A の fresh pairing 代表ケースが `1 passed in 33.27s`。trace は direct `0x30`、30秒の観測窓完了、close 開始、終了時 neutral `0x30`、transport close、cleanup の順序を記録した。利用者が close 前に画面反応とコントローラー認識を目視確認した。postflight は HCI / CSR identity の一致と `adapter_closed` を確認。先行した active reconnect 2件は認証失敗で Button A 送信前に終了したため、この判定には使用しない
- unit_067 default battery status: 2026-07-24 に Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 / swbt-python 0.4.0 で pass。保存済み adapter-default profile から active reconnect し、5秒間の neutral report loop を実行した。outgoing `0x30` 501件と `0x21` 16件は全 PDU の battery / connection byte が `0x80`。advertising / pairing / key update / trace error は0件で、利用者は Switch UI が充電中または充電機器接続中を示さない期待どおりの表示になったことを目視確認した。終了時は neutral `0x30` 後の disconnect terminal が0.25秒 timeout したが、transport close と adapter release は完了した
- unit_066 adapter-default profile gate: 2026-07-24 に Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 / swbt-python 0.4.0 で pass。`local_address` を省略し、adapter が `power_on()` 後に報告した current public address `0E:08:71:C0:B4:5C` を namespace にして fresh pairing と active reconnect を実行した。profile は `identity.kind="adapter-default"` で address を保存せず、`adapter_identity_prepared` は 0 件。reconnect は profile bytes 不変、advertising / pairing / key update 0 件。両 run とも `transport_close_complete` を記録した。fresh close では neutral 送信時に `ClosedError: Bumble interrupt channel is not connected` を1件記録しており、終了時 neutral report の到達は未確認
- Issue #88 multi-address round trip: 2026-07-21 に Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 / swbt-python 0.4.0 / Switch 2（firmware はこの run では未記録）で `tests/hardware/test_multi_address_reconnect.py::test_switch_two_profiles_round_trip_without_repairing` を実行した。A=`0E:08:71:C0:B4:5C`、B=`0E:08:71:C0:B4:5D` を volatile に適用し、fresh profile pairing を A/B 各1回、active reconnect を A→B→A→B の4回実行。pytest は `1 passed, 1 warning in 22.71s`。各 reconnect は `connected`、profile bytes 不変、`classic_pairing` / `advertising_start` / `key_store_update` 0件。Switch UI 上で A/B 両登録が残ったかの目視はこの記録では未確認。artifact は `tmp/hardware/unit_058/issue-88-round-trip-2`。各 session は neutral close、最後に dongle を通常 close した。pytest 終了時に Bumble USB callback の `USBTransfer.__callbackWrapper` 後処理 warning を1件観測したが、シーケンス判定は pass
- Issue #88 post-run recovery: 2026-07-21 に read-only `csr_bd_addr_probe.py --hci-reset` を実行し、standard HCI / CSR の双方が B=`0E:08:71:C0:B4:5D` を返し、status `0`、一致、adapter close。volatile address は通常 close / HCI Reset では復帰しないため、dongle physical power cycle 後の read-only recovery を保留して追加操作を停止した
- Issue #88 post-power-cycle recovery: 2026-07-21 に dongle 抜き差し後の read-only `csr_bd_addr_probe.py --hci-reset` を実行。standard HCI / CSR の双方が元の `00:1B:DC:F9:9F:7D` を返し、status `0`、一致、adapter close。A/B 実験後の volatile address が physical power cycle で元 identity に復帰することを observed-pass とした
- Issue #88 five-second observation window: 2026-07-22 に既存 A/B profile を再利用し、`tests/hardware/test_multi_address_reconnect.py::test_switch_two_profiles_observation_window_after_reconnect` を `usb:0` で実行。A に reconnect → 5秒待機 → neutral close、B に同じ手順を実行し、pytest は `1 passed in 18.06s`。両方とも `connected`、profile bytes 不変、pairing / advertising / key-store update なし。artifact は `tmp/hardware/unit_058/issue-88-round-trip-2/multi-address-a-observe.jsonl` と `multi-address-b-observe.jsonl`。Switch UI 上の A/B 登録残存は利用者目視確認待ち
- Issue #88 UI confirmation and final recovery: 2026-07-22 に利用者が Switch UI 上で A/B の登録残存と接続成功を目視確認し、dongle を抜き差しした。続く read-only `csr_bd_addr_probe.py --hci-reset` は standard HCI / CSR とも元の `00:1B:DC:F9:9F:7D`、status `0`、一致、adapter close。Issue #88 の実機完了条件を満たした
...
- CSR BD_ADDR dummy-address Switch pairing: 2026-07-20 に local-address rerun 後の物理復旧を確認し、`00:11:22:33:44:55` をvolatile適用した。fresh key storeの初回runはprotocol pairing / HID / full initial subcommand列 / clean closeがpassしたが、Switch UIは反応なし。ユーザがdongleを抜き差しした後のreuse attemptはpreflightで元addressを検出し、advertising前に安全停止した。dummy addressを再適用したreuse rerunはprotocol passし、ユーザがSwitch UIで登録されたことを目視確認した。dummy addressが対象Switchの登録経路に受理されることをobserved-passとする。warm reset直後のUSB transfer警告は再列挙と整合し、後続のstandard HCI / CSR / Bumble address一致により適用成功を判定した
- CSR BD_ADDR dummy-address post-pair recovery: 2026-07-20 にユーザがdongleを抜き差しした後、HCI Resetを含むread-only recovery probeがpass。standard HCI / CSR default-storeはともに元の`00:1B:DC:F9:9F:7D`、CSR status `0`、一致、`adapter_closed`。dummy-address pairing後もphysical power cycleで元identityへ復帰することをobserved-passとした
- CSR BD_ADDR dummy-address normal-close retention: 2026-07-20 にdummy addressをvolatile再適用し、既存key storeでpairing、neutral report 5秒、通常close、同一process内のraw adapter再openとHCI Resetなしのidentity readを連続実行した。close後もstandard HCI / CSR default-storeはともに`00:11:22:33:44:55`、CSR status `0`、一致、`post_close_matches_expected=true`。対象個体ではBumble通常closeをまたいでvolatile active BD_ADDRが保持されることをobserved-passとした。続く物理power cycle後はstandard HCI / CSRが元の`00:1B:DC:F9:9F:7D`へ復帰し、clean closeした
- unit_052 exp local profile gate: 2026-07-20 に Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 で pass。乱数生成した個別・ローカル管理アドレス `0E:08:71:C0:B4:5C` を volatile 適用し、fresh profile pairing / normal close と、同一 profile の `already_active` active reconnect を確認した。reconnect では profile bytes が変わらず、advertising / pairing / key update / error は 0 件。Switch 2 / firmware 22.5.0 は利用者確認済み
...
- cleanup: 各pairing processはL2CAP channel close、disconnect request、`transport_close_complete`、result `controller_and_adapter_closed`。reuse guard failureもpreflight adapter close。最後のdummy pairing後、physical power cycleとread-only recovery readを実行し元identityへの復帰を確認済み
- notes: warm reset enqueue直後の `OUT transfer not completed: status=4` とIN transfer未完了はUSB再列挙時に古いhandleで観測される警告。単独では成功根拠にせず、別processのstandard HCI / CSR / Bumble address一致で適用成功と判定した。ユーザ補足により、fresh初回runとreuse attemptの間にdongleを抜き差ししていた。元addressへの復帰はphysical power cycleの結果であり、transport closeだけでvolatile addressが戻る根拠にはしない。Bumble 0.0.230 `Device.power_off()`がhost flushのみで明示HCI Resetを送らないことはsource factだが、この時点では通常closeをまたぐvolatile address保持は未検証

### 2026-07-20: CSR BD_ADDR dummy-address post-pair recovery

- OS: Windows 11 `10.0.26200`
- environment: branch `experiment/bd-addr-rewrite`、commit `f10b761`、実行前worktree clean
- adapter: ユーザが抜き差しした専用 `usb:0`
- dongle: CSR8510 A10、VID:PID `0a12:0001`
- driver: WinUSB
- Python: `3.13.5`
...
- Switch firmware: not used
- report period: not applicable
- command / test: `uv run python tools/csr_bd_addr_probe.py --adapter usb:0 --hci-reset --timeout 2 --output tmp/hardware/unit_051/dummy-address-post-pair-recovery.json`
- approval: ユーザがdongle抜き差し後の復旧確認を依頼。範囲は専用`usb:0`、HCI Reset、standard identity read、CSR GETREQ、close。PSKEY write、advertising、pairingは対象外
- result: pass。standard HCI / CSR default-store addressはともに元の`00:1B:DC:F9:9F:7D`。company identifier `10`、CSR status `0`、`matches_standard_hci=true`
- artifact: `tmp/hardware/unit_051/dummy-address-post-pair-recovery.json`
- cleanup: `adapter_closed`
- notes: dummy-address Switch登録後のvolatile identityはphysical power cycleで元identityへ復帰した。専用adapterは実験前のaddress状態へ復旧済み

### 2026-07-20: CSR BD_ADDR dummy-address normal-close retention

- OS: Windows 11 `10.0.26200`
- environment: branch `experiment/bd-addr-rewrite`、commit `b6b101e`、実行前worktree clean
- adapter: 専用 `usb:0`
- dongle: CSR8510 A10、VID:PID `0a12:0001`
- driver: WinUSB
- Python: `3.13.5`
...
- Switch model: not re-confirmed in this run
- Switch firmware: not re-confirmed in this run
- report period: default `8000 us`、neutral inputのみ、接続後observation `5.0 s`
- command / test: `uv run python tools/csr_bd_addr_warm_reset_probe.py --adapter usb:0 --expected-original 00:1B:DC:F9:9F:7D --requested-address 00:11:22:33:44:55 --execute --timeout 2 --settle-seconds 0.5 --output tmp/hardware/unit_051/dummy-address-close-retention-warm-reset-apply.json`、続けて `uv run python tools/csr_bd_addr_switch_pair_probe.py --adapter usb:0 --expected-address 00:11:22:33:44:55 --key-store tmp/hardware/unit_051/dummy-address-switch-keys.json --reuse-key-store --trace tmp/hardware/unit_051/dummy-address-close-retention.jsonl --output tmp/hardware/unit_051/dummy-address-close-retention-result.json --execute --preflight-timeout 2 --pair-timeout 60 --observation-seconds 5 --post-close-address-read`。物理power cycle後に `uv run python tools/csr_bd_addr_probe.py --adapter usb:0 --hci-reset --timeout 2 --output tmp/hardware/unit_051/dummy-address-close-retention-post-power-cycle-recovery.json`
- approval: ユーザが専用 `usb:0` のvolatile dummy address再適用、CSR warm reset、既存dummy key store再利用、HID advertising / pairing、neutral report loop 5秒、通常close、直後のHCI Resetなしread-only identity確認を明示承認。不揮発書き込みとbutton inputは対象外。実験後にユーザがdongleを抜き差しし、承認済みcleanupとしてHCI Reset、standard HCI / CSR read、closeを実行した
- result: pass。PSRAM SETREQ status `0`、dummy address read-back、warm reset enqueueを確認した。pairing前preflightはstandard HCI / CSRが`00:11:22:33:44:55`で一致。Classic pairing、HID connected、initial subcommand 16件、neutral report 125件、clean closeがpassした。通常close直後の同一process readもstandard HCI / CSRが`00:11:22:33:44:55`、CSR status `0`、`matches_standard_hci=true`、`post_close_matches_expected=true`。物理power cycle後はstandard HCI / CSRが元の`00:1B:DC:F9:9F:7D`、CSR status `0`、一致
- artifact: `tmp/hardware/unit_051/dummy-address-close-retention-warm-reset-apply.json`、`tmp/hardware/unit_051/dummy-address-close-retention-result.json`、`tmp/hardware/unit_051/dummy-address-close-retention.jsonl`、`tmp/hardware/unit_051/dummy-address-close-retention-post-power-cycle-recovery.json`
- cleanup: pairing probeはdisconnect request、L2CAP channel close、`transport_close_complete`を記録した。post-close raw probeも`adapter_closed`、全体resultは`controller_and_post_close_adapter_closed`。物理power cycle後のrecovery probeも`adapter_closed`。専用adapterは元addressへ復旧済み
- notes: 物理抜き差しと明示HCI Resetはpairing開始からpost-close read完了まで介在していない。これは対象CSR8510 A10 / WinUSB / Bumble 0.0.230でのhardware observationである。通常closeとraw adapter再openは連続しているため、別構成への一般化はせず、両者を個別要因として分離した観測でもない
...
### 2026-07-20: unit_052 exp local profile fresh pairing / normal close / active reconnect

- OS: Windows 11、`Windows-11-10.0.26200-SP0`
- environment: Windows PowerShell、`feat/unit-052-exp-local-address-profile` branch
- adapter: `usb:0`。no-open 列挙時の bus 6 / device 49
- dongle: CSR8510 A10、USB VID:PID `0A12:0001`
- driver: WinUSB service、libwdi provider。Windows PnP の読み取り専用情報で実行直前に確認
- Python: 3.13.5
- Bumble: 0.0.230
- Switch model / firmware: run 後に利用者が Switch 2 / firmware 22.5.0 と確認
- exp local address: 利用者の承認を受けて Codex が乱数生成した個別・ローカル管理アドレス `0E:08:71:C0:B4:5C`。6 octet、個別、ローカル管理、予約 inquiry LAP 外を `ExpLocalAddress.parse()` で確認
- report period: 8000 us default
- first command / test: `uv run pytest tests/hardware/test_exp_local_profile.py::test_switch_exp_local_profile_fresh_pairing_and_close -m hardware --swbt-bumble-adapter usb:0 --swbt-exp-local-address 0E:08:71:C0:B4:5C --swbt-hardware-artifact-dir build/hardware/exp-local-profile-20260720 -q -s`
- retry command / test: `uv run pytest tests/hardware/test_exp_local_profile.py::test_switch_exp_local_profile_fresh_pairing_and_close -m hardware --swbt-bumble-adapter usb:0 --swbt-exp-local-address 0E:08:71:C0:B4:5C --swbt-hardware-artifact-dir build/hardware/exp-local-profile-20260720-retry1 -q -s`
- approval: user explicitly approved address generation and the hardware run. Scope included dedicated `usb:0` open、current address read、必要時の CSR PSRAM write / warm reset / USB 再列挙 / read-back、Bumble HID advertising、Switch pairing、periodic report loop、neutral、normal close。persistent write と close 時の address restore は対象外
- result: first run は volatile address を factory value `00:1B:DC:F9:9F:7D` から target へ書き換え、Classic pairing、key store update、HID connected、normal close まで到達した。ただし hardware test が初期化時点では未確定の `bumble_device_initialized.local_bluetooth_address` を要求したため assertion fail。実 trace に合わせ、`bumble_device_initialized` の存在と、power-on 後の `local_bluetooth_address_configured.address` を別々に検査するよう修正した。既存 unit test 45件が pass。retry は `1 passed in 2.95s`
- retry observation: `exp_local_identity_prepared status=already_active current_address=target_address=0E:08:71:C0:B4:5C` のため、最初の normal close 後も target が保持され、retry では volatile write と warm reset を省略した。trace は `local_bluetooth_address_configured address=0e0871c0b45c`、`advertising_start`、`classic_pairing`、`key_store_update status=succeeded`、`connected`、`transport_close_complete` を記録した
- artifact: `build/hardware/exp-local-profile-20260720/exp-local-fresh-pairing.jsonl`、`build/hardware/exp-local-profile-20260720/exp-local-pro-profile.json`、`build/hardware/exp-local-profile-20260720-retry1/exp-local-fresh-pairing.jsonl`、`build/hardware/exp-local-profile-20260720-retry1/exp-local-pro-profile.json`。profile の link key 値はログへ転記しない
- cleanup: 両 run とも `transport_close_complete` を記録し、adapter を解放した。retry trace では Switch 切断後の best-effort neutral 中に `ClosedError: Bumble interrupt channel is not connected` を記録したが、close は正常完了した。後続 active reconnect gate のため物理 power cycle は行わず、volatile target を意図的に保持した
- active reconnect command / test: `uv run pytest tests/hardware/test_exp_local_profile.py::test_switch_exp_local_profile_reuses_target_after_normal_close -m hardware --swbt-bumble-adapter usb:0 --swbt-exp-local-address 0E:08:71:C0:B4:5C --swbt-hardware-artifact-dir build/hardware/exp-local-profile-20260720-retry1 -q -s`
- active reconnect approval: user explicitly approved the listed command while waiting on a connectable Switch screen. Scope included dedicated `usb:0` open、current address read、target guard、Bumble power-on、active reconnect、periodic report loop、normal close。advertising、intentional pairing、key update、address restore は対象外
- active reconnect result: pass、`1 passed in 2.88s`。trace は `exp_local_identity_prepared status=already_active current_address=target_address=0E:08:71:C0:B4:5C`、`local_bluetooth_address_configured address=0e0871c0b45c`、`active_reconnect_result route=active_reconnect status=connected`、`transport_close_complete` を記録した。`advertising_start`、`classic_pairing`、`key_store_update`、`error` は各 0 件
- active reconnect profile evidence: test が reconnect 前後の profile bytes 完全一致を検査して pass。run 後 SHA-256 は `880F9949F16BA6980F2A5657740BD3C6C3F1229D18D46744877BF6E08BFC4858`。link key 値は記録しない
- active reconnect artifact: `build/hardware/exp-local-profile-20260720-retry1/exp-local-active-reconnect.jsonl` と同 directory の `exp-local-pro-profile.json`
- active reconnect cleanup: `pad.close(neutral=True)` を実行し、trace は `transport_close_complete` を記録した。error event は 0 件。adapter は解放した。address restore と物理 power cycle は実行していない
- notes: fresh profile 作成、pairing key 保存、normal close、同一 profile の active reconnect、profile bytes 不変、advertising / pairing / key update の非実行を hardware-pass とする。Switch 2 / firmware 22.5.0 は利用者確認済みである。別 model / firmware へ一般化しない

### 2026-07-20: unit_053 Joy-Con exp local profile fresh pairing / normal close / active reconnect

- OS: Windows 11、`Windows-11-10.0.26200-SP0`
- environment: Windows PowerShell、`feat/unit-053-joycon-exp-profile` branch、実行時は未コミット変更あり
- adapter: `usb:0`。no-open 列挙時の bus 6 / device 53
- dongle: CSR8510 A10、USB VID:PID `0A12:0001`
- driver: WinUSB service、libwdi provider。Windows PnP の読み取り専用情報で実行直前に確認
- Python: 3.13.5
- Bumble: 0.0.230
- Switch model / firmware: Switch 2 / 22.5.0。利用者が実行前に確認
- report period: 8000 us default
- Joy-Con L exp local address: `12:78:D1:C4:E5:03`。個別・ローカル管理・予約 inquiry LAP 外を `ExpLocalAddress.parse()` で確認
- Joy-Con L fresh command / test: `uv run pytest 'tests/hardware/test_exp_local_profile.py::test_switch_joycon_exp_local_profile_fresh_pairing_and_close[left-JoyConL-joycon_l]' -m hardware --swbt-bumble-adapter usb:0 --swbt-exp-local-address 12:78:D1:C4:E5:03 --swbt-hardware-artifact-dir build/hardware/unit-053-joycon-l-20260720 -q -s`
- Joy-Con L fresh result: pass、`1 passed in 5.31s`。factory address `00:1B:DC:F9:9F:7D` から target へ rewrite、Classic pairing、key store update、normal close を記録。profile は `controller_kind=joycon_l`、SHA-256 は `2A0AD65273C77E7FCB8A5BA382754D46EE01DF8553624B1B2701C1E05EA856E3`
- Joy-Con L reconnect command / test: `uv run pytest 'tests/hardware/test_exp_local_profile.py::test_switch_joycon_exp_local_profile_reuses_target_after_normal_close[left-JoyConL-joycon_l]' -m hardware --swbt-bumble-adapter usb:0 --swbt-exp-local-address 12:78:D1:C4:E5:03 --swbt-hardware-artifact-dir build/hardware/unit-053-joycon-l-20260720 -q -s`
- Joy-Con L reconnect result: pass、`1 passed in 3.25s`。current/target はともに `12:78:D1:C4:E5:03`、active reconnect connected、advertising / Classic pairing / key store update は各 0 件。profile bytes は test が完全一致を確認
- Joy-Con R exp local address: `6E:9B:54:87:29:79`。個別・ローカル管理・予約 inquiry LAP 外を `ExpLocalAddress.parse()` で確認
- Joy-Con R fresh command / test: `uv run pytest 'tests/hardware/test_exp_local_profile.py::test_switch_joycon_exp_local_profile_fresh_pairing_and_close[right-JoyConR-joycon_r]' -m hardware --swbt-bumble-adapter usb:0 --swbt-exp-local-address 6E:9B:54:87:29:79 --swbt-hardware-artifact-dir build/hardware/unit-053-joycon-r-20260720 -q -s`
- Joy-Con R fresh result: pass with warning、`1 passed, 1 warning in 4.68s`。L target から R target へ rewrite、Classic pairing、key store update、normal close を記録。profile は `controller_kind=joycon_r`、SHA-256 は `D0872D20398E1C2E22E91E7EAC30A23933AF1300ABCD1254FEBC35840D939D03`
- Joy-Con R reconnect command / test: `uv run pytest 'tests/hardware/test_exp_local_profile.py::test_switch_joycon_exp_local_profile_reuses_target_after_normal_close[right-JoyConR-joycon_r]' -m hardware --swbt-bumble-adapter usb:0 --swbt-exp-local-address 6E:9B:54:87:29:79 --swbt-hardware-artifact-dir build/hardware/unit-053-joycon-r-20260720 -q -s`
- Joy-Con R reconnect result: pass、`1 passed in 2.96s`。current/target はともに `6E:9B:54:87:29:79`、active reconnect connected、advertising / Classic pairing / key store update は各 0 件。profile bytes は test が完全一致を確認
- approval: 利用者が Switch 2 / 22.5.0 での実行準備完了と、dedicated `usb:0` open、CSR volatile read/write / warm reset / USB 再列挙、Bumble HID advertising、Switch pairing、periodic neutral report loop、active reconnect、normal close を明示承認。persistent write、address restore、物理 power cycle は対象外
- artifact: `build/hardware/unit-053-joycon-l-20260720/` と `build/hardware/unit-053-joycon-r-20260720/` の fresh / reconnect trace と profile。link key 値は本ログへ転記しない
- cleanup: 全4 run で `pad.close(neutral=True)` を実行し、trace は `transport_close_complete` を記録。adapter は各 run 後に解放。最終的な volatile address は Joy-Con R target のまま保持し、restore と物理 power cycle は実行していない
- warnings / notes: 全4 trace に Switch 切断後の best-effort neutral 中の `ClosedError: Bumble interrupt channel is not connected` が1件あるが、close は完了した。R fresh pytest は teardown 時に Bumble / `usb1` の ctypes callback が解放済み transfer を参照する `PytestUnraisableExceptionWarning` を1件出した。いずれも unit_053 の pairing / reconnect 判定を覆すものではないが、Bumble / usb1 teardown の未解決観測として残す。別 OS、adapter、Switch firmware への一般化はしない

### 2026-07-21: unit_054 Direct exp local profile fresh pairing / send / active reconnect

- OS: Windows 11、`Windows-11-10.0.26200-SP0`
- environment: Windows PowerShell、`feat/unit-054-direct-exp-profile` branch、未コミット変更あり
- adapter / dongle / driver: dedicated `usb:0`、CSR8510 A10、VID:PID `0A12:0001`、Windows PnP status `OK`。WinUSB 構成は unit_053 と同じ既知構成
- Python / Bumble: 3.13.5 / 0.0.230
- Switch model / firmware: Switch 2 / 22.5.0。利用者が接続待機画面を準備し、adapter open、volatile address write、HID advertising、pairing、Direct `send()`、active reconnect、normal close を明示承認
- Direct Pro: `02:54:10:20:30:41`。fresh pairing / `send(Button.A)` は `1 passed in 5.26s`、active reconnect / `send(Button.A)` は `1 passed in 1.93s`。profile SHA-256: `979626490798ACEBD6B9018BCA51A210670E295694D3B6B81157FDFB95CB4479`
- Direct Joy-Con L: `02:54:10:20:30:43`。fresh pairing / `send(Button.L)` は `1 passed in 5.28s`、active reconnect / `send(Button.L)` は `1 passed in 2.04s`。profile SHA-256: `D490F1C709270163F19A3D9C8CFC26BF18BCDF802E7BA06F6862C87BEEEA8DD5`
- Direct Joy-Con R: `02:54:10:20:30:45`。fresh pairing / `send(Button.R)` は `1 passed in 5.30s`、active reconnect / `send(Button.R)` は `1 passed in 1.91s`。profile SHA-256: `0A1C656C1499C5EF2E6B742913C03F446E9F139F2C45252F9B7554A03B2EADF9`
- artifact: `build/hardware/unit-054-direct-pro-20260721/`、`build/hardware/unit-054-direct-joycon-l-20260721/`、`build/hardware/unit-054-direct-joycon-r-20260721/`。link key 値は転記しない
- cleanup: 全6 run で `pad.close(neutral=True)` を実行し `transport_close_complete` を確認。adapter は各 run 後に解放。volatile address は最後の Direct Joy-Con R target のまま。address restore と物理 power cycle は未実施
- notes: fresh trace 3件には Switch 切断後の best-effort neutral 中の `ClosedError: Bumble interrupt channel is not connected` が各1件あるが、close は完了。別 OS、adapter、Switch firmware へ一般化しない

### 2026-07-22: Issue #93 Direct send timing characterization

- OS: Windows 11 `Windows-11-10.0.26200-SP0`
- environment: `feat/issue-93`、commit `8a91219`。使い捨て harness は `tmp/hardware/issue-93/direct_send_timing_benchmark.py`
- adapter: `usb:0`
- dongle: CSR8510 A10、USB VID:PID `0A12:0001`、instance `USB\VID_0A12&PID_0001\9&12127A34&0&1`
- driver: WinUSB
- Python / Bumble / swbt-python: 3.13.5 / 0.0.230 / 0.4.0
- Switch model / firmware: この run では再確認していない
- report period: Direct caller が 8 ms 500件、16 ms 250件、sleep なし直列 burst 300件を全て neutral state で送信。DEBUG attribution は 8 ms / 16 ms 各100件
- command / test: `uv run python tools/csr_bd_addr_probe.py --adapter usb:0 --output tmp/hardware/issue-93/adapter-identity-preflight.json`、`uv run python tmp/hardware/issue-93/direct_send_timing_benchmark.py pair --adapter usb:0 --profile-path tmp/hardware/issue-93/direct-pro-profile.json --local-address 0E:08:71:C0:B4:5C --artifact-dir tmp/hardware/issue-93 --pair-timeout 10`、`uv run python tmp/hardware/issue-93/direct_send_timing_benchmark.py benchmark --adapter usb:0 --profile-path tmp/hardware/issue-93/direct-pro-profile.json --artifact-dir tmp/hardware/issue-93 --reconnect-timeout 60`
- approval: ユーザが専用 `usb:0`、volatile local address、warm reset、HID advertising、Switch pairing、active reconnect、neutral Direct send、終了時 neutral / close / adapter release を明示承認。不揮発書込み、button、stick input は対象外
- result: read-only preflight は factory address / CSR default-store ともに `00:1B:DC:F9:9F:7D` で一致。10秒 pairing は `classic_pairing`、key store update、HID control / interrupt channel、connected まで到達。benchmark reconnect は `transport_close_complete` まで完了。logger 無効の baseline で `send()` duration は 8 ms 500件の median 30.008 ms、p95 45.047 ms、p99 60.305 ms、16 ms 250件の median 30.006 ms、p95 30.966 ms、p99 60.005 ms。両 cadence の completion deadline miss は全件。直列 burst は 300件 / 9.345006秒、32.10 completed/s。DEBUG event 200件は lock wait が input median 0.6 us、sender median 1.3 us、HID enqueue median 31.0 us、ACL drain median 29.772 ms、pending が毎回 `0 -> 1 -> 0`、outcome は全件 success と記録
- artifact: `tmp/hardware/issue-93/adapter-identity-preflight.json`、`pairing-trace.jsonl`、`direct-pro-profile.json`、`benchmark-summary.json`、`direct-send-timing.jsonl`、`reconnect-trace.jsonl`。profile の link key は転記しない
- cleanup: pairing harness の初回は trace close 順序不具合により clean-close 記録を残せなかった。修正後 benchmark run は `close(neutral=True)`、disconnect request、0.25秒 terminal timeout 後の disconnected、`transport_close_complete` を記録し adapter を解放。後続 read-only probe は active local address / CSR default-store ともに `0E:08:71:C0:B4:5C`、`adapter_closed` を記録。address restore と physical power cycle は未実施
- notes: Bumble / usb1 は pairing process 終了時に callback exception、benchmark process 終了時に `no pending response future to set` を stderr へ出した。benchmark の lifecycle trace は error event を記録していない。約30 ms はこの OS / CSR8510 / Bumble / Switch 接続の観測であり、一般的な latency guarantee ではない

### 2026-07-22: Issue #93 HCI ACL completion correlation

- OS: Windows 11 `Windows-11-10.0.26200-SP0`
- environment: `feat/issue-93`、未コミットの unit_060 probe 拡張。Python 3.13.5 / Bumble 0.0.230 / swbt-python 0.4.0
- adapter / dongle / driver: dedicated `usb:0`、CSR8510 A10、VID:PID `0A12:0001`、WinUSB。preflight / postflight とも active address / CSR PSKey address は `0E:08:71:C0:B4:5C` で一致し、cleanup は `adapter_closed`
- Switch model / firmware: この run では再確認していない。既存 `direct-pro-profile.json` の paired peer へ active reconnect した
- command / test: `tools/csr_bd_addr_probe.py` による read-only preflight / postflight と、`tmp/hardware/issue-93/direct_send_timing_benchmark.py benchmark --adapter usb:0 --profile-path tmp/hardware/issue-93/direct-pro-profile.json --artifact-dir tmp/hardware/issue-93/hci-completion-correlation --reconnect-timeout 10 --paced-8ms-samples 10 --paced-16ms-samples 10 --burst-samples 20 --diagnostic-8ms-samples 20 --diagnostic-16ms-samples 20`
- approval: ユーザが `usb:0` の Bumble adapter open、active reconnect、neutral Direct send、DEBUG artifact 出力、neutral close、adapter release と postflight を承認。新規 pairing、button / stick input、BD_ADDR 書換えは対象外
- result: baseline 8 ms は 10/10 deadline miss、send p50 9.992 ms。16 ms は 0/10 miss、send p50 8.823 ms。DEBUG event 40 件は全件 success、packet count は全件 1。`acl_completion_event_delay_ns` は p50 9.757 ms / p95 12.220 ms、`acl_drain_duration_ns` は p50 9.746 ms / p95 12.217 ms、両者の差は p50 17.6 us / p95 32.6 us。Bumble 0.0.230 の `HCI_Mode_Change_Event.Mode` では raw mode `2` は `SNIFF`、event の enqueue / completion snapshot は全件 mode `2`、raw interval `8`
- artifact: `tmp/hardware/issue-93/hci-completion-correlation/adapter-identity-preflight.json`、`adapter-identity-postflight.json`、`benchmark-summary.json`、`direct-send-timing.jsonl`、`reconnect-trace.jsonl`
- cleanup: benchmark harness が `close(neutral=True)` を実行。trace に `transport_close_complete`、postflight に `adapter_closed` を記録。physical power cycle と address restore は実施していない
- notes: benchmark 終了時に Bumble が `no pending response future to set` を stderr へ出したが、trace に error event はない。preflight でも `DISCONNECTION COMPLETE: unknown handle` を stderr へ出したが probe は passed。SNIFF snapshot と約10 ms の HCI completion 観測は、sniff mode が遅延原因であることを単独では証明しない。前回の約30 ms 観測との差は、この run の記録だけでは原因を確定できない

### 2026-07-22: Issue #93 Classic link policy A/B stop condition

- OS / environment: Windows 11 `Windows-11-10.0.26200-SP0`、`feat/issue-93` の未コミット unit_061 experimental helper。Python 3.13.5 / Bumble 0.0.230 / swbt-python 0.4.0
- adapter / dongle / driver: dedicated `usb:0`、CSR8510 A10、VID:PID `0A12:0001`、WinUSB。preflight と recovery 後 postflight は `0E:08:71:C0:B4:5C` が HCI / CSR PSKey で一致し、`adapter_closed`
- Switch model / firmware: この run では再確認していない。既存 `direct-pro-profile.json` の paired peer を active reconnect した
- command / test: `direct_send_timing_benchmark.py benchmark` を `--link-policy allow-sniff` (`0x0005`) と `--link-policy role-switch-only` (`0x0001`) で実行。各 run は neutral Direct send、`reconnect-timeout 10`、8 ms 20件、16 ms 100件、burst 20件、DEBUG 8 ms 20件、16 ms 100件を指定。candidate 失敗後、recovery は `allow-sniff` / 8 ms 5件 / 16 ms 10件に短縮した
- approval: ユーザが `usb:0` の Bumble adapter open、policy A/B、active reconnect、neutral Direct send、DEBUG artifact、neutral close、adapter release、postflight を承認。新規 pairing、button / stick input、BD_ADDR 書換えは対象外
- result: `allow-sniff` は trace で `classic_link_policy_configured: 0x0005`、DEBUG 120 event は全件 success。completion delay は p50 9.894 ms / p95 12.604 ms、enqueue snapshot は全件 Bumble mode `2` (`SNIFF`) / raw interval `8`。`role-switch-only` は trace で `0x0001` を確認し、active reconnect、authentication、encryption、HID control / interrupt channel open、connected まで到達したが、neutral `0x30` report 3件後に reason `19` (`HCI_REMOTE_USER_TERMINATED_CONNECTION_ERROR`) で切断された。benchmark summary は生成されず、A/B の反復と latency 比較を停止した。recovery `allow-sniff` は reconnect、neutral send、`transport_close_complete` を確認
- artifact: `tmp/hardware/issue-93/link-policy-ab/adapter-identity-preflight.json`、`01-allow-sniff/`、`02-role-switch-only/reconnect-trace.jsonl`、`recovery-allow-sniff/`、`adapter-identity-postflight.json`、`adapter-identity-post-recovery.json`
- cleanup: candidate / recovery harness は `close(neutral=True)` を実行。candidate trace は `transport_close_complete`、recovery trace も `transport_close_complete`。recovery 後 postflight は `adapter_closed`。physical power cycle と address restore は未実施
- notes: `0x0001` はこの OS / CSR8510 / Bumble / paired Switch 接続で stable neutral Direct send を維持できない hardware observation。sniff 許可 bit が切断の唯一原因であること、他 adapter / OS / firmware へ一般化できることは未検証。各 harness process の private reference policy は `finally` で復元し、後続 `allow-sniff` trace が `0x0005` を記録した

### 2026-07-22: Issue #93 Periodic ACL drain characterization

- OS / environment: Windows 11 `Windows-11-10.0.26200-SP0`、`feat/issue-93` の未コミット unit_062 probe。Python 3.13.5 / Bumble 0.0.230 / swbt-python 0.4.0
- adapter / dongle / driver: dedicated `usb:0`、CSR8510 A10、VID:PID `0A12:0001`、WinUSB。実行直前 preflight と最終 postflight は HCI / CSR PSKey address ともに `0E:08:71:C0:B4:5C`、postflight cleanup は `adapter_closed`
- Switch model / firmware: この run では再確認していない。既存 `direct-pro-profile.json` の paired peer へ `ProController` で active reconnect した。Periodic / Direct 間の同一 controller shape profile 再利用は、この run で接続・periodic report 送信まで確認できた
- command / test: `uv run python tmp/hardware/issue-93/periodic_send_timing_benchmark.py --adapter usb:0 --profile-path tmp/hardware/issue-93/direct-pro-profile.json --artifact-dir tmp/hardware/issue-93/periodic-acl-drain/02-8ms-rerun --reconnect-timeout 10 --report-period-us 8000 --samples 40 --sample-timeout 10` と同一 command の `--artifact-dir .../03-16ms --report-period-us 16000`。終了後に `uv run python tools/csr_bd_addr_probe.py --adapter usb:0 --output tmp/hardware/issue-93/periodic-acl-drain/adapter-identity-postflight.json`
- approval: ユーザが `usb:0` の Bumble adapter open、active reconnect、HID advertising / channel open、neutral-only periodic report の 8 ms / 16 ms 各40件、neutral close、adapter release、postflight を承認。新規 pairing、button / stick input、不揮発書込み、address restore は対象外
- result: 8 ms は periodic record 40件、scheduler sleep p50 8.594 ms、completion interval p50 20.023 ms / p95 30.105 ms、transport send p50 11.455 ms、ACL drain p50 11.386 ms、HCI completion event delay p50 11.411 ms。16 ms は periodic record 40件、sleep p50 16.839 ms、completion interval p50 25.116 ms / p95 35.029 ms、transport send p50 8.456 ms、ACL drain p50 8.385 ms、HCI completion delay p50 8.424 ms。両 run で enqueue は microsecond order、ACL drain と transport send はほぼ同じ値で、configured sleep と transport send の和がおおむね completion interval になった
- artifact: `tmp/hardware/issue-93/periodic-acl-drain/02-8ms-rerun/`、`03-16ms/`、`adapter-identity-postflight.json`。初回 `01-8ms/` は subcommand reply まで periodic record と数える harness 不具合により periodic sample 3件しか含まないため結果から除外し、artifact は調査履歴として残す
- cleanup: 各有効 run は `close(neutral=True)`、disconnect request、0.25秒 terminal timeout 後の disconnected、`transport_close_complete` を記録。trace の error event は 0件。最終 read-only postflight は `adapter_closed`。physical power cycle と address restore は実施していない
- notes: harness process は各 run 終了時に Bumble stderr `no pending response future to set` を出したが、有効 run の trace に error はない。ここで測った completion interval は Python の periodic report 完了時刻であり、Switch UI の入力反映時刻ではない。CSR8510 A10 / WinUSB / Bumble 0.0.230 / この paired Switch 接続に限る観測であり、他環境への latency guarantee ではない

### 2026-07-23: Issue #93 Periodic no-drain characterization

- OS / environment: Windows 11 `Windows-11-10.0.26200-SP0`、`feat/issue-93` の未コミット調査ハーネス。Python 3.13.5 / Bumble 0.0.230 / swbt-python 0.4.0
- adapter / dongle / driver: dedicated `usb:0`、CSR8510 A10、USB VID:PID `0A12:0001`、WinUSB。preflight / postflight はHCI / CSR PSKey addressともに `0E:08:71:C0:B4:5C`、cleanupは `adapter_closed`
- Switch model / firmware: このrunでは再確認していない。既存 `direct-pro-profile.json` のpaired peerへactive reconnectした
- report period: 8000 us。接続成立後の短時間だけ `drain_bumble_acl_queue` をenqueue-onlyのno-opへ差し替え、neutral Periodic `0x30` を200件収集した
- command / test: preflight `uv run python tools/csr_bd_addr_probe.py --adapter usb:0 --output tmp/hardware/issue-93/periodic-no-drain/adapter-identity-preflight.json`。有効run `uv run python tmp/hardware/issue-93/periodic_send_timing_benchmark.py --adapter usb:0 --profile-path tmp/hardware/issue-93/direct-pro-profile.json --artifact-dir tmp/hardware/issue-93/periodic-no-drain/02-8ms --reconnect-timeout 10 --report-period-us 8000 --samples 200 --sample-timeout 10 --acl-completion-policy enqueue-only`。postflightは同じidentity probeを `adapter-identity-postflight.json` へ出力
- approval: ユーザがdrainを外した検証を依頼した。実行範囲はdedicated `usb:0` open、既存bondのactive reconnect、neutral-only Periodic 125 Hzを最大200件、ACL queue private state観測、通常drain復元、neutral close、adapter release、postflight。新規pairing、button / stick / IMU、BD_ADDR書換えは対象外
- result: 有効runはPeriodic timing 200件を収集。report completion intervalはmin 8.235 ms / p50 8.979 ms / p95 10.054 ms / p99 10.227 ms / max 10.672 ms、transport sendはp50 0.087 ms / p95 0.150 ms。unit_062のdrainあり8 ms runはcompletion interval p50 20.023 ms、transport send p50 11.455 msだった
- ACL flow result: controller申告値は `max_packet_size=310`、`max_in_flight=10`。観測中の最大connection / controller in-flightとpendingはいずれも3、Bumble software queue depthは全transitionで0。completion event 220件はすべてpacket count 1で、開始時に残っていた1件と観測中enqueue 219件を回収し、observer終了時はqueued total 225 / completed total 225 / pending 0 / in-flight 0だった
- shared reply observation: active reconnect完了後にも初期subcommand 16件を受信し、共有interrupt経路の `subcommand_reply_tx` 16件もno-drain窓に含まれた。traceはreply送信を全件記録し、観測窓中のunexpected disconnectと `error` eventはなかった。Switchによる意味的受理と長時間安定性はこのrunでは確定しない
- first-run correction: `01-8ms` はactive reconnect後、ACL queue解決を直接属性だけに仮定したためno-drain適用前に停止した。通常drainのままneutral close、`transport_close_complete` を確認した。production helperと同じ `device.host.get_data_packet_queue(handle)` fallbackを追加して有効runだけを再実行した
- artifact: `tmp/hardware/issue-93/periodic-no-drain/adapter-identity-preflight.json`、`01-8ms/`、`02-8ms/benchmark-summary.json`、`02-8ms/periodic-send-timing.jsonl`、`02-8ms/acl-queue-transitions.jsonl`、`02-8ms/reconnect-trace.jsonl`、`adapter-identity-postflight.json`
- cleanup: no-drain差し替えを先に通常drainへ復元し、`close(neutral=True)` を実行。ACL queue artifactは最終pending / in-flight 0、traceは `transport_close_complete`、postflightは `adapter_closed` を記録した。physical power cycleとaddress restoreは実施していない
- notes: 有効run終了時もBumble stderrに既知の `no pending response future to set` が1件出たが、traceの `error` eventは0件。125 Hz設定でも現在のschedulerは各send完了後に8 ms sleepするため、no-drainの実周期p50は8.979 ms（約111 Hz）であり、厳密な125 Hz deadline schedulerではない。この値を他adapter / OS / Switch firmwareへ一般化しない

### 2026-07-23: Issue #93 production no-drain 5-minute soak

- OS / environment: Windows 11 `Windows-11-10.0.26200-SP0`、`feat/issue-93` の未コミットproduction enqueue-completion実装。Python 3.13.5 / Bumble 0.0.230 / swbt-python 0.4.0
- adapter / dongle / driver: dedicated `usb:0`、CSR8510 A10、VID:PID `0A12:0001`、WinUSB。preflight / postflightはHCI / CSR PSKey addressともに `0E:08:71:C0:B4:5C`
- Switch model / firmware: このrunでは再確認していない。既存 `direct-pro-profile.json` のpaired peerへactive reconnectした
- report period: production `ProController`、8000 us、neutral-only、300秒
- command / test: preflight / postflightは `uv run python tools/csr_bd_addr_probe.py --adapter usb:0 --output tmp/hardware/issue-93/production-no-drain-soak/adapter-identity-{preflight,postflight}.json`。soakは `uv run python tmp/hardware/issue-93/production_no_drain_soak.py --adapter usb:0 --profile-path tmp/hardware/issue-93/direct-pro-profile.json --artifact-dir tmp/hardware/issue-93/production-no-drain-soak/01-5min --duration-seconds 300 --reconnect-timeout 10 --report-period-us 8000`
- approval: ユーザが専用 `usb:0`、既存bondへのactive reconnect、8 msのneutral Periodic、通常subcommand応答、5分への時間短縮、`close(neutral=True)`、adapter解放を明示承認。新規pairing、button / stick / IMU入力、BD_ADDR書換え、不揮発書込みは対象外
- result: pass。301.721秒で完了し、300秒の観測終了直前まで `connection_state=connected`、`last_error=null`。periodic `0x30` は33159件、subcommand rx / replyは各16件、trace `error` eventは0件。close時にneutral `0x30` を1件受理し、disconnect requestはchannels `[interrupt, control]` / status `requested`、その後reason 0のdisconnectedと `transport_close_complete`を記録した
- artifact: `tmp/hardware/issue-93/production-no-drain-soak/adapter-identity-preflight.json`、`01-5min/reconnect-trace.jsonl`、`01-5min/soak-summary.json`、`adapter-identity-postflight.json`
- cleanup: `close(neutral=True)`はclose errorなし。disconnect request terminal waitは0.25秒でtimeoutを記録した後、reason 0のdisconnectedを受けてtransport closeまで完了。postflightはidentity一致と `adapter_closed`を確認した。physical power cycleとaddress restoreは実施していない
- notes: process開始時にBumble既知stderr `no pending response future to set` が1件あったが、soak traceのerror eventは0件。約33159件 / 300秒は約110.5 report/sで、固定sleepに処理時間が加算されるIssue #103の範囲と整合するが、このrunは精密な周期計測ではない。単一adapter / OS / paired Switch接続の安定性観測であり、他環境へ一般化しない

### 2026-07-23: Issue #103 Periodic deadline scheduler timing

- OS: Windows 11 `Windows-11-10.0.26200-SP0`
- environment: `codex/issue-103-report-deadline`、commit `50bba8e`。Python 3.13.5 / Bumble 0.0.230 / swbt-python 0.4.0
- adapter / dongle / driver: dedicated `usb:0`、CSR8510 A10、WinUSB。preflight / postflight は HCI / CSR PSKey address ともに `0E:08:71:C0:B4:5C` で一致
- Switch model / firmware: この run では再確認していない。既存 `direct-pro-profile.json` の paired peer へ active reconnect した
- report period: production `ProController`、8000 us、neutral-only、Periodic `0x30` 200件
- command / test: read-only preflight / postflight は `tools/csr_bd_addr_probe.py --adapter usb:0 --output tmp/hardware/issue-103/adapter-identity-{preflight,postflight}.json`。計測は `tmp/hardware/issue-103/periodic_deadline_timing_benchmark.py --adapter usb:0 --profile-path tmp/hardware/issue-93/direct-pro-profile.json --artifact-dir tmp/hardware/issue-103/periodic-deadline-8ms-01 --baseline-summary tmp/hardware/issue-93/periodic-no-drain/02-8ms/benchmark-summary.json --implementation-commit 50bba8e --reconnect-timeout 10 --report-period-us 8000 --samples 200 --sample-timeout 10`
- approval: ユーザが Switch のペアリング画面で待機中であることを示し、dedicated `usb:0` open、read-only identity、既存 bond active reconnect、HID channel、通常 subcommand 応答、neutral-only Periodic 200件、neutral close、adapter release、postflight を明示承認。新規 pairing、advertising、button / stick / IMU、BD_ADDR 書換えは対象外
- result: pass。200件を取得し、completion interval は min 5.971 ms / p50 8.038 ms / p95 9.093 ms / p99 9.978 ms。旧 scheduler の同一設定観測 p50 8.979 ms / p95 10.054 ms に対し、p50 は 0.942 ms短縮して8 msへ近づいた。transport sendはp50 0.021 ms / p95 0.043 ms。connection statusは `connected`、trace error event 0件、subcommand reply 16件
- outlier: 199 interval中2件が20 ms超で、index 6は952.384 ms、index 10は711.700 ms。trace上、この位置では初期subcommand reply 16件が2群に分かれて送信され、既存の periodic holdoff をまたいだ。p50 / p95の改善判定には影響しないが、steady cadenceだけの最大値として扱わない
- artifact: `tmp/hardware/issue-103/adapter-identity-preflight.json`、`periodic-deadline-8ms-01/benchmark-summary.json`、`periodic-send-timing.jsonl`、`reconnect-trace.jsonl`、`adapter-identity-postflight.json`
- cleanup: benchmark は `close(neutral=True)` を実行し、close errorなし、traceに `transport_close_complete` を記録。postflight はidentity一致と `adapter_closed` を確認。physical power cycle と address restore は実施していない
- notes: benchmark終了時にBumble既知stderr `no pending response future to set` が1件出たが、trace error eventは0件。結果は単一adapter、1接続、短時間の観測であり、Switch側反映時刻や他環境の125 Hz保証ではない

### 2026-07-24: unit_066 adapter-default profile fresh pairing / active reconnect

- OS: Windows 11 `Windows-11-10.0.26200-SP0`
- environment: `codex/pairing-profile-default-address` worktree、未コミットの unit_066 実装。Python 3.13.5 / Bumble 0.0.230 / swbt-python 0.4.0
- adapter: dedicated `usb:0`、no-open 列挙時 bus 6 / device 91 / port 9.1
- dongle: CSR8510 A10、VID:PID `0A12:0001`、serial number なし
- driver: 既存の WinUSB 構成。driver assignment 自体はこの run では再確認していない
- Switch model: この run では再確認していない。同じ peer address の既存記録では Switch 2
- Switch firmware: この run では再確認していない。同じ peer address の 2026-07-20 記録では 22.5.0
- report period: 既定 8000 us、neutral-only
- command / test: `uv run pytest tests/hardware/test_pairing_profile.py::test_switch_adapter_default_profile_fresh_pairing_and_close -m hardware --swbt-bumble-adapter usb:0 --swbt-hardware-artifact-dir build/hardware/unit_066/adapter-default-20260723 --log-file build/hardware/unit_066/adapter-default-20260723/fresh-pairing-pytest-debug.log --log-file-level=DEBUG -q -s`
- command / test: `uv run pytest tests/hardware/test_pairing_profile.py::test_switch_adapter_default_profile_reuses_address_after_normal_close -m hardware --swbt-bumble-adapter usb:0 --swbt-hardware-artifact-dir build/hardware/unit_066/adapter-default-20260723 --log-file build/hardware/unit_066/adapter-default-20260723/reconnect-pytest-debug.log --log-file-level=DEBUG -q -s`
- approval: ユーザが専用 `usb:0` の adapter open、fresh Classic HID pairing / advertising、保存 bond への active reconnect、neutral periodic report、`close(neutral=True)`、adapter release を明示承認。non-neutral input と BD_ADDR 書換えは対象外
- result: fresh pairing は `1 passed in 2.98s`。profile は `identity.kind="adapter-default"` で address field を持たず、current public address `0E:08:71:C0:B4:5C` の namespace に key を保存した。trace は `adapter_identity_prepared` 0 件、`advertising_start`、`classic_pairing`、`key_store_update status=succeeded`、HID control / interrupt open、`connected` を記録した
- result: active reconnect は `1 passed in 4.52s`。同じ current public address と namespace を選び、profile bytes 不変、`active_reconnect_result status=connected`。`adapter_identity_prepared`、`advertising_start`、`classic_pairing`、`key_store_update` は各 0 件
- artifact: `build/hardware/unit_066/adapter-default-20260723/pairing-profile-adapter-default-fresh-pairing.jsonl`、`pairing-profile-adapter-default-reconnect.jsonl`、`fresh-pairing-pytest-debug.log`、`reconnect-pytest-debug.log`、`pairing-profile-adapter-default-pro.json`
- cleanup: fresh / reconnect とも `transport_close_complete` を記録し、pytest process は正常終了した。reconnect は neutral input report 後に disconnect request、terminal timeout 0.25秒、reason 0 の disconnected、transport close。fresh は `close(neutral=True)` 中に `ClosedError: Bumble interrupt channel is not connected` を1件記録した後、control / interrupt close、disconnect request、reason 0 の disconnected、transport closeまで完了した
- notes: fresh close の `ClosedError` により終了時 neutral report の到達は確認できない。ただし test は non-neutral input を送っていない。current public address は以前の volatile 書換値が給電断なしで残った値であり、factory / baseline address ではない。adapter-default が「その時点で controller が報告した値」を使うという仕様と一致する。別 adapter、別 OS、給電断後に current address が変わる場合、Direct / Periodic 間の profile 再利用は未検証

### 2026-07-24: unit_067 default battery status `0x80`

- OS: Windows 11 `Windows-11-10.0.26200-SP0`
- environment: `fix/battery-status-0x80` branch、未コミットの unit_067 実装。Python 3.13.5 / Bumble 0.0.230 / swbt-python 0.4.0
- adapter: dedicated `usb:0`。unit_066 の adapter-default profile と current public address namespace を再利用
- dongle: CSR8510 A10、VID:PID `0A12:0001`。unit_066 と同じ個体
- driver: 既存の WinUSB 構成。driver assignment 自体はこの run では再確認していない
- Switch model: この run では再確認していない。同じ peer address の既存記録では Switch 2
- Switch firmware: この run では再確認していない。同じ peer address の 2026-07-20 記録では 22.5.0
- report period: 既定 8000 us、neutral-only、接続後5秒
- command / test: `uv run pytest tests/hardware/test_battery_status.py::test_switch_reports_default_battery_status_for_manual_ui_confirmation -m hardware --swbt-bumble-adapter usb:0 --swbt-hardware-artifact-dir build/hardware/unit_067/battery-status-20260724 --log-file build/hardware/unit_067/battery-status-20260724/battery-status-pytest-debug.log --log-file-level=DEBUG -q -s -p no:cacheprovider --basetemp C:\Users\train\AppData\Local\Temp\swbt-unit-067-hardware-a`
- approval: ユーザが専用 `usb:0` の adapter open、Bumble Classic HID 初期化、保存済み key による active reconnect、HID control / interrupt channel、5秒の periodic neutral report、Switch UI 目視確認、neutral close を明示承認。advertising、新規 pairing、key update、non-neutral input は対象外
- result: `1 passed in 6.83s`。`active_reconnect_result route=active_reconnect status=connected`。trace は outgoing `0x30` 501件、`0x21` 16件を記録し、Bumble debug log の該当 PDU も全件 battery / connection byte `0x80` だった。advertising / pairing / key update / trace error は各0件。profile bytes は不変
- user observation: Switch UI が充電中または充電機器接続中を示さず、期待どおりの表示になったことを利用者が目視確認した。この結果は本 run の Switch model / firmware 条件付き hardware observation であり、別 firmware へ一般化しない
- artifact: `build/hardware/unit_067/battery-status-20260724/battery-status.jsonl`、`battery-status-pytest-debug.log`、`battery-status-profile.json`
- cleanup: 5秒の観測後に `close(neutral=True)` を実行し、最後の `0x30` reason `input` を送信した。disconnect request は `requested`、terminal wait は0.25秒で `timeout` したが、その後 `transport_close_complete` を記録し、pytest process は正常終了した。non-neutral input は送っていない
- notes: `0x80` は満充電、非充電、外部給電なしを表す固定値であり、実際の電池残量を動的に測定した結果ではない。disconnect terminal timeout は battery status の失敗ではなく、transport close 完了と分けて記録する

### 2026-07-24: unit_068 Direct fresh pairing close 前入力反映

- OS: Windows 11 `Windows-11-10.0.26200-SP0`
- environment: `codex/test-direct-pre-close-input-reflection` branch、未コミットの unit_068 実装。Python 3.13.5 / Bumble 0.0.230 / swbt-python 0.4.0
- adapter / dongle / driver: dedicated `usb:0`、CSR8510 A10、VID:PID `0A12:0001`、no-open 列挙時 bus 6 / device 91 / port 9.1、WinUSB
- Switch model / firmware: Switch 2 / 22.5.0
- command / test: preflight は `uv run python tools/csr_bd_addr_probe.py --adapter usb:0 --output build\hardware\unit_068\direct-pro-fresh-pre-close-20260724-115129\adapter-identity-preflight.json`
- command / test: `uv run pytest 'tests/hardware/test_pairing_profile.py::test_switch_direct_pairing_profile_fresh_pairing_holds_input_before_close[pro-DirectProController-pro-Button.A]' -m hardware --swbt-bumble-adapter usb:0 --swbt-local-address 0E:08:71:C0:B4:5C --swbt-hardware-artifact-dir build\hardware\unit_068\direct-pro-fresh-pre-close-20260724-115129 --log-file build\hardware\unit_068\direct-pro-fresh-pre-close-20260724-115129\pytest-debug.log --log-file-level=DEBUG -q -s -p no:cacheprovider --basetemp C:\tmp\swbt-unit-068-fresh-20260724-115129`
- approval: 利用者が Switch の「持ちかた／順番を変える」画面を準備し、dedicated `usb:0` open、identity preparation、HID advertising、fresh pairing、Direct Button A 1件、30秒の close 前観測窓、neutral close、adapter release を明示承認した。firmware 変更、persistent write、address restore、Joy-Con variant は対象外
- result: `1 passed in 33.27s`。profile は `controller_kind=pro`、local address `0E:08:71:C0:B4:5C`、SHA-256 `04DAF41ED1E872ED076D95A43B5B541C013584229E06E37805311C173902685A`
- trace result: `operator_prepare_input_observation` index 0、connected 21、Button A の direct `0x30` 23、`direct_input_enqueued` 24、`pre_close_observation_window_complete` 71、`close_start` 72、終了時 neutral direct `0x30` 73、disconnect request 98、`transport_close_complete` 103、closed cleanup 104。pytest の順序 assertion が pass した
- user observation: 利用者が30秒の観測窓中、close 前に Switch の画面反応とコントローラーとしての認識を目視確認した。pytest pass と分けた hardware observation として observed-pass とする
- preliminary reconnect attempts: 同じ作業中の active reconnect 2件は、artifact `build/hardware/unit_068/direct-pro-pre-close-20260724-114134/` で HCI authentication failure reason 5 により Button A 送信前に失敗した。いずれも adapter は close した。fresh pairing test の acceptance gap に範囲を絞ったため再試行せず、close 前 input reflection の成否判定には使用しない
- artifact: `build/hardware/unit_068/direct-pro-fresh-pre-close-20260724-115129/` の pairing trace、profile、pytest debug log、identity preflight / postflight。link key 値は転記しない
- cleanup: `close(neutral=True)` は終了時 neutral report、disconnect request、transport close、closed cleanup を記録した。read-only postflight は HCI / CSR address がともに `0E:08:71:C0:B4:5C` で一致し、`adapter_closed` を確認。physical power cycle と address restore は実施していない
- notes: この結果は単一の Direct Pro / Button A、対象 adapter、OS、Switch firmware の観測である。Joy-Con L / R parameter は収集済みだが実機未実行であり、別環境へ一般化しない

### 2026-07-24: unit_069 Pro Controller handshake readiness

- OS: Windows 11 `Windows-11-10.0.26200-SP0`
- environment: `codex/handshake-readiness-design` branch、commit `f11850a` と未コミットの neutral initialization pump 修正。Python 3.13.5 / Bumble 0.0.230 / swbt-python 0.5.0
- adapter / dongle / driver: dedicated `usb:0`、CSR8510 A10、VID:PID `0A12:0001`、bus 6 / device 91 / port 9.1、WinUSB (`oem75.inf`、libwdi)
- Switch model / firmware: この run では再確認していない。同じ peer の過去記録は Switch 2 / 22.5.0 だが、本 run の確定条件としては扱わない
- report period: 既定 8000 us、neutral-only
- command / test: fresh pairing は `uv run pytest tests/hardware/test_pairing_profile.py::test_switch_adapter_default_profile_fresh_pairing_and_close -q -m hardware --basetemp tmp/hardware/unit_069/pro-handshake-readiness-20260724/pytest-fresh --swbt-bumble-adapter usb:0 --swbt-hardware-artifact-dir tmp/hardware/unit_069/pro-handshake-readiness-20260724`
- command / test: 修正前 reconnect は同じ artifact directory と `pytest-reconnect` basetemp、通常画面 retry は `reconnect-home-retry-1` を使用。修正後 reconnect は `reconnect-home-retry-2` を使用して `test_switch_adapter_default_profile_reuses_address_after_normal_close` を実行
- approval: 利用者が Pro Controller を先行対象として、dedicated `usb:0` open、fresh pairing / advertising、保存 bond の active reconnect、Switch-facing subcommand handling、neutral report、neutral close、adapter release を明示承認。non-neutral input、Joy-Con、BD_ADDR 書換えは対象外
- initial result: fresh pairing は Classic pairing、link key、key store update、HID control / interrupt channel まで成功したが、`protocol_initialization_started` 後に subcommand 0件のまま60秒 timeout。保存 key による active reconnect と通常画面での retry も link / HID channel までは成功したが、同じく subcommand 0件で timeout
- diagnosis: 初回実装は Periodic `ReportLoop` を protocol ready 後に開始していた。3 run とも link 確立後に outgoing input report がなく、Switch が初期 subcommand 列を開始しない deadlock と一致した
- characterization result: 原因切り分けのため Periodic loop を link 接続時に開始し、initializing 中は利用者 state ではなく neutral `0x30` を送ると、active reconnect は `1 passed in 3.60s`
- trace result: `protocol_initialization_started` 後に neutral periodic `0x30` を6件送ると、Switch は `0x02`、`0x08`、`0x10`、`0x03`、`0x04`、`0x40`、`0x30 00`、`0x48`、`0x21`、`0x30 01` を送信した。`0x03` は `report_mode=0x30`、最初の player lights は `0x00` で ready false、最後の player lights は `0x01` で ready true。最後の `subcommand_reply_tx` 後に `protocol_ready`、その後に `active_reconnect_result status=connected`
- artifact: `tmp/hardware/unit_069/pro-handshake-readiness-20260724/`。修正前 fresh / reconnect trace と、修正後 `reconnect-home-retry-2/pairing-profile-adapter-default-reconnect.jsonl` を保存
- cleanup: 全 run で `transport_close_complete` を記録。修正後は終了時 neutral `0x30`、disconnect request、reason 0 の disconnected、transport close まで完了。non-neutral input は送っていない
- implementation follow-up: 常時loopは必要範囲が広いため採用せず、neutral起動 reportをsubcommand未受信中だけ1秒間隔で再送し、最初の有効なsubcommand parse後に停止する方式へ縮小した。この方式はlocal fake integrationで確認済みだが、このhardware log時点では実機未再検証
- reply-only result: 縮小版 active reconnect は60秒timeout。起動report 2件後の`0x02`でbootstrap停止、replyだけで`0x03 30`、`0x04`、追加SPIまで進んだが、`0x40`とplayer lightsへ進まなかった。artifactは`bootstrap-reply-driven-reconnect-1`
- final result: `0x03 30` reply後をhost-requested report modeとしてneutral periodicを開始する版は`1 passed in 4.44s`。bootstrapは2件、最初の`0x02`で停止。`0x03 30`後のperiodic 2件を挟んで`0x40`、`0x30 00`、`0x48`、`0x21`、`0x30 01`へ進み、最後のreply後に`protocol_ready`、続いて`active_reconnect_result status=connected`
- final artifact: `tmp/hardware/unit_069/pro-handshake-readiness-20260724/report-mode-driven-reconnect-1/pairing-profile-adapter-default-reconnect.jsonl`
- asserted hardware gate: fresh / reconnect testへbootstrap停止、全受信subcommandへのreply、`0x03 30`、nonzero player lights、reply直後のreadyを検査するassertionを追加した。保存profileを複製した`report-mode-driven-reconnect-asserted-1`でreconnectを再実行し、`1 passed in 4.98s`
- asserted artifact: `tmp/hardware/unit_069/pro-handshake-readiness-20260724/report-mode-driven-reconnect-asserted-1/pairing-profile-adapter-default-reconnect.jsonl`
- fresh ready gate: Switch側の登録削除後、adapter-default fresh pairingをready trace assertion付きで再実行し、`1 passed in 5.39s`。key store update後、bootstrap `0x30` 2件を最初の`0x02`で停止し、`0x03 30`、`0x30 00`、`0x30 01`を観測した。`0x30 01` reply直後に`protocol_ready`となり、routeは`pairing`
- fresh artifact: `tmp/hardware/unit_069/pro-fresh-ready-20260724-1/pairing-profile-adapter-default-fresh-pairing.jsonl`
- fresh cleanup: 終了時neutral `0x30`、disconnect request、terminal close、reason 0 の disconnected、`transport_close_complete`を記録。non-neutral入力とBD_ADDR書換えは実施していない
- Direct ready-stop gate: fresh pairing profileを複製し、Direct Proでactive reconnect。`1 passed in 5.44s`。ready前は`0x03 30` reply後のneutral periodicを2件記録し、`0x30 01` reply直後にready。ready後の1秒間はautomatic periodic 0件で、終了時neutralだけ`reason=direct`として1件送信
- Direct artifact: `tmp/hardware/unit_069/direct-pro-ready-stop-20260724-1/pairing-profile-direct-adapter-default-reconnect.jsonl`
- Direct cleanup: disconnect terminalは0.25秒timeoutしたが、reason 0 の disconnectedと`transport_close_complete`を記録。profile bytes不変、advertising / pairing / key updateなし、non-neutral入力なし
- Joy-Con L ready gate: Switch側のPro登録削除後、adapter-default profileでfresh pairing `1 passed in 5.15s`、同profileのactive reconnect `1 passed in 5.94s`。どちらもbootstrap 2件を最初の`0x02`で停止し、Joy-Con L profileの`0x04` reply後、`0x03 30`、`0x30 00`、`0x48`、`0x30 01`へ進み、最後のreply直後に`profile_kind=joycon_left`でready
- Joy-Con L `0x04`: 実装とunit fixtureはSL / SRを各300 tickとするreply data `00000000000000002c012c010000`。hardware traceは`0x04`受信とreply完了後、追加のSR+SL input reportなしでnonzero player lightsへ到達した
- Joy-Con L artifact: `tmp/hardware/unit_069/joycon-l-ready-20260724-1/`のfresh / reconnect JSONLとadapter-default profile
- Joy-Con L cleanup: freshはdisconnect terminal closed、reconnectは0.25秒timeout後にreason 0のdisconnected。両runとも`transport_close_complete`、neutral close、profile bytes不変を確認。BD_ADDR書換え、non-neutral入力なし
- Joy-Con R ready gate: 利用者がProとJoy-Con Lを含むSwitch側登録を削除後、adapter-default profileでfresh pairing `1 passed in 5.52s`、同profileのactive reconnect `1 passed in 6.46s`。どちらもbootstrap 2件を最初の`0x02`で停止し、Joy-Con R profileの`0x04` reply後、`0x03 30`、`0x30 00`、`0x48`、optional `0x22`、`0x30 08`へ進み、最後のreply直後に`profile_kind=joycon_right`でready
- Joy-Con R `0x04`: 実装とunit fixtureはSL / SRを各300 tickとするreply data `00000000000000002c012c010000`。hardware traceは`0x04`受信とreply完了後、追加のSR+SL input reportなしでnonzero player lightsへ到達した。`0x22`はfresh / reconnectとも観測してreplyしたが、固定completion gateにはしていない
- Joy-Con R artifact: `tmp/hardware/unit_069/joycon-r-ready-20260724-1/`のfresh / reconnect JSONLとadapter-default profile
- Joy-Con R cleanup: freshはdisconnect terminal closed、reconnectは0.25秒timeout後にreason 0のdisconnected。両runとも`transport_close_complete`、neutral close、profile bytes不変を確認。BD_ADDR書換え、non-neutral入力なし
- final cleanup: 終了時neutral `0x30`、disconnect request、reason 0 の disconnected、`transport_close_complete`を記録。non-neutral入力なし
- remaining: unit_069の予定したPro Controller、Direct Pro、Joy-Con L、Joy-Con R実機gateは完了。Switch側には最後にfresh pairingしたJoy-Con Rだけが登録され、先行Pro / Joy-Con L登録は利用者が削除済み
