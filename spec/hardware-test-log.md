...
- command / test: `uv run python tools/csr_bd_addr_warm_reset_probe.py --adapter usb:0 --expected-original 00:1B:DC:F9:9F:7D --requested-address 00:11:22:33:44:55 --execute --timeout 2 --settle-seconds 0.5 --output tmp/hardware/unit_051/dummy-address-close-retention-warm-reset-apply.json`、続けて `uv run python tools/csr_bd_addr_switch_pair_probe.py --adapter usb:0 --expected-address 00:11:22:33:44:55 --key-store tmp/hardware/unit_051/dummy-address-switch-keys.json --reuse-key-store --trace tmp/hardware/unit_051/dummy-address-close-retention.jsonl --output tmp/hardware/unit_051/dummy-address-close-retention-result.json --execute --preflight-timeout 2 --pair-timeout 60 --observation-seconds 5 --post-close-address-read`
- approval: ユーザが専用 `usb:0` のvolatile dummy address再適用、CSR warm reset、既存dummy key store再利用、HID advertising / pairing、neutral report loop 5秒、通常close、直後のHCI Resetなしread-only identity確認を明示承認。不揮発書き込みとbutton inputは対象外
- result: pass。PSRAM SETREQ status `0`、dummy address read-back、warm reset enqueueを確認した。pairing前preflightはstandard HCI / CSRが`00:11:22:33:44:55`で一致。Classic pairing、HID connected、initial subcommand 16件、neutral report 125件、clean closeがpassした。通常close直後の同一process readもstandard HCI / CSRが`00:11:22:33:44:55`、CSR status `0`、`matches_standard_hci=true`、`post_close_matches_expected=true`
- artifact: `tmp/hardware/unit_051/dummy-address-close-retention-warm-reset-apply.json`、`tmp/hardware/unit_051/dummy-address-close-retention-result.json`、`tmp/hardware/unit_051/dummy-address-close-retention.jsonl`
- cleanup: pairing probeはdisconnect request、L2CAP channel close、`transport_close_complete`を記録した。post-close raw probeも`adapter_closed`、全体resultは`controller_and_post_close_adapter_closed`。dongleはvolatile dummy addressのままなので、物理power cycleとread-only recovery確認が必要
