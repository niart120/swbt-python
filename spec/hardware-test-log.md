...
- unit_069 Pro Controller handshake readiness: 2026-07-24 に Windows 11 / `usb:0` / CSR8510 A10 / WinUSB / Bumble 0.0.230 / Python 3.13.5 で実行。reportなしはsubcommand 0件でtimeout、最初のsubcommandで全自動送信を止めるreply-only版は`0x03 30`後の追加SPIでtimeoutした。最終版はneutral起動reportを最初の`0x02`まで2件送り、`0x03 30` reply後だけneutral periodicを開始。`0x30 01` reply後にreadyとなった。ready trace assertion追加後のactive reconnectは`1 passed in 4.98s`。cleanup完了。fresh pairingとDirect実機は未完了
...
- remaining: Switch側の保存済みcontroller情報を削除したfresh pairingと、Directがready後に自動`0x30`を停止する実機確認。Pro Controller Periodic reconnectだけpass
