...
- Python: 3.13.5
- Bumble: 0.0.230
- Switch model / firmware: この run では記録していない
...
- notes: fresh profile 作成、pairing key 保存、normal close、同一 profile の active reconnect、profile bytes 不変、advertising / pairing / key update の非実行を hardware-pass とする。Switch model / firmware はこの run で記録していないため、その構成へ一般化しない
